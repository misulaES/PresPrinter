//********* !!  ojo !! hay modificaciones para MULTIMAT_PRINTER !!!!!
/**
...
uint8_t lcd_status_message_level;    //  Higher level blocks lower level
  ...
  constexpr bool first_page = true;
	...
	extern  int obturador_pwm_max;  //valor pwm para el L298N (1 a 255)
	extern int t_max_obturador_ms = MS_LIMITE_OBTURADOR;  //tiempo maximo de operacion cierre del obturador en milisegundos 
	extern int t_min_entre_operaciones_obturador_ms = MS_MIN_ENTRE_OPER_OBTURADOR;  //Tiempo en miliseg minimo entre operaciones
#endif
...
uint16_t max_display_update_time = 0;
  ...
  DEFINE_LCD_IMPLEMENTATION_DRAWMENU_SETTING_EDIT_TYPE(float, float52, ftostr52);
    ...
    DEFINE_LCD_IMPLEMENTATION_DRAWMENU_SETTING_EDIT_TYPE(int16_t, int4, itostr4); 
    DEFINE_LCD_IMPLEMENTATION_DRAWMENU_SETTING_EDIT_TYPE(float, float53, ftostr43);
  #endif 
  ...
  #if DISABLED(NO_VOLUMETRICS) || ENABLED(ADVANCED_PAUSE_FEATURE)|| ENABLED(PASTAPRINT)  //Añadido UVPRINT
  ...
  DECLARE_MENU_EDIT_TYPE(float, float3);
    ...
    DECLARE_MENU_EDIT_TYPE(float, float53);
  #endif
 ...
 void lcd_sdcard_pause() {
    ...
    void lcd_sdcard_stop() {
      ...
      card.abort_sd_printing = true;
		...
		// Baja la cama FIN_IMPRESION_BAJAR_MM_CAMA mm 
		do_blocking_move_to_z( MIN(current_position[Z_AXIS]+ FIN_IMPRESION_BAJAR_MM_CAMA, Z_MAX_POS), NOZZLE_PARK_Z_FEEDRATE); 
		point_t punto = FIN_IMPRESION_MOVER_A_XY ;
		do_blocking_move_to_xy(punto.x, punto.y,NOZZLE_PARK_XY_FEEDRATE);	
		planner.synchronize();
	#endif	  

	  #if ENABLED(PASTAPRINT)

		//Si la temperatura es mayor de 30? pongo a funcionar el ventilador del hotend.
		if (thermalManager.current_temperature[0] > 30 ){
			thermalManager.setTargetHotend(0,0);
			thermalManager.setTargetBed(0);			
			enqueue_and_echo_commands_P(PSTR("M106 S255"));
			planner.synchronize();		
			do {
				stop_mensaje(MSG_ENFRIANDO, 5);
			}while (thermalManager.current_temperature[0] > 30 );
			
			planner.synchronize();
			enqueue_and_echo_commands_P(PSTR("M107")); //apago el ventilador
		}
	  #endif
  ...
  void lcd_control_menu() {
    ...
    #if DISABLED(NO_VOLUMETRICS) || ENABLED(ADVANCED_PAUSE_FEATURE) || ENABLED(PASTAPRINT)
   ...
   */
    ...
    #if ENABLED(PIDTEMP)
		#if ENABLED(PASTAPRINT)
//			int16_t autotune_temp[HOTENDS] = ARRAY_BY_HOTENDS1(50);  //con esto sale 75 en vez de 50
			int16_t autotune_temp[HOTENDS] = { 50 };
		#else
  ...
  void lcd_control_temperature_menu() {
    ...
    void lcd_control_motion_velocity_menu() {
      ...
      #if ENABLED(DISTINCT_E_FACTORS)
    //    MENU_MULTIPLIER_ITEM_EDIT(float43, MSG_VMAX MSG_E, &planner.max_feedrate_mm_s[E_AXIS + active_extruder], 0.05f, 9.0f);
        MENU_MULTIPLIER_ITEM_EDIT(float43, MSG_VMAX MSG_E, &planner.max_feedrate_mm_s[E_AXIS], 1, 999);
        MENU_MULTIPLIER_ITEM_EDIT(float43, MSG_VMAX MSG_E1, &planner.max_feedrate_mm_s[E_AXIS + 1], 1, 999);
        #if E_STEPPERS > 2
        ...
        #endif // E_STEPPERS > 2
      #else
        MENU_MULTIPLIER_ITEM_EDIT(float43, MSG_VMAX MSG_E, &planner.max_feedrate_mm_s[E_AXIS], 0.1f, 9.9f);
      #endif

      // M205 S Min Feedrate
      MENU_MULTIPLIER_ITEM_EDIT(float43, MSG_VMIN, &planner.min_feedrate_mm_s, 10, 999);

      // M205 T Min Travel Feedrate
      MENU_MULTIPLIER_ITEM_EDIT(float43, MSG_VTRAV_MIN, &planner.min_travel_feedrate_mm_s, 10, 999);
    ...
    void lcd_control_motion_acceleration_menu() {
      ...
      #if ENABLED(DISTINCT_E_FACTORS)
  //      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(long5, MSG_AMAX MSG_E, &planner.max_acceleration_mm_per_s2[E_AXIS + active_extruder], 5, 99000, _reset_acceleration_rates);
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(long5, MSG_AMAX MSG_E, &planner.max_acceleration_mm_per_s2[E_AXIS], 100, 99000, _reset_e0_acceleration_rate);
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(long5, MSG_AMAX MSG_E1, &planner.max_acceleration_mm_per_s2[E_AXIS + 1], 100, 99000, _reset_e1_acceleration_rate);
        #if E_STEPPERS > 2
        ...
        #endif // E_STEPPERS > 2
      #else
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(long5, MSG_AMAX MSG_E, &planner.max_acceleration_mm_per_s2[E_AXIS], 5, 99000, _reset_acceleration_rates);
      #endif
    ...
    void lcd_control_motion_steps_per_mm_menu() {
      ...
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_ASTEPS, &planner.axis_steps_per_mm[A_AXIS], 5, 9999, _planner_refresh_positioning);
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_BSTEPS, &planner.axis_steps_per_mm[B_AXIS], 5, 9999, _planner_refresh_positioning);
      MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_CSTEPS, &planner.axis_steps_per_mm[C_AXIS], 5, 9999, _planner_refresh_positioning);

      #if ENABLED(DISTINCT_E_FACTORS)
 //       MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_ESTEPS, &planner.axis_steps_per_mm[E_AXIS + active_extruder], 5, 9999, _planner_refresh_positioning);
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_ESTEPS, &planner.axis_steps_per_mm[E_AXIS], 5, 9999, _planner_refresh_e0_positioning);
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_E1STEPS, &planner.axis_steps_per_mm[E_AXIS + 1], 5, 9999, _planner_refresh_e1_positioning);
        #if E_STEPPERS > 2
          MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_E3STEPS, &planner.axis_steps_per_mm[E_AXIS + 2], 5, 9999, _planner_refresh_e2_positioning);
          #if E_STEPPERS > 3
            MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_E4STEPS, &planner.axis_steps_per_mm[E_AXIS + 3], 5, 9999, _planner_refresh_e3_positioning);
            #if E_STEPPERS > 4
              MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float5, MSG_E5STEPS, &planner.axis_steps_per_mm[E_AXIS + 4], 5, 9999, _planner_refresh_e4_positioning);
            #endif // E_STEPPERS > 4
          #endif // E_STEPPERS > 3
        #endif // E_STEPPERS > 2
      #else
        MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float51, MSG_ESTEPS, &planner.axis_steps_per_mm[E_AXIS], 5, 9999, _planner_refresh_positioning);
      #endif
  ...
  void lcd_control_motion_menu() {
    ...
    void lcd_control_filament_menu() {
      ...
      MENU_BACK(MSG_CONTROL);
        ...
        MENU_ITEM_EDIT(float52, MSG_ADVANCE_K, &planner.extruder_advance_K, 0, 9);
      #endif
        ...
        if (parser.volumetric_enabled) {
			#if ENABLED(PASTAPRINT)
			MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float52, "Diam."MSG_NOZZLE , &planner.diam_boquilla, 0.40, 3.00, planner.calculate_volumetric_multipliers);
			MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(int8, MSG_PORC_RENDMTO_EXTRUSION, &planner.factor_rendimiento_extrusion, 1, 200,  planner.actualiza_steps_mm3_E);
			#endif			
         #if EXTRUDERS == 1
            MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float52, MSG_DIAM_EMBOLO, &planner.filament_size[0],1, 50,  planner.actualiza_steps_mm3_E);

          #else // EXTRUDERS > 1
            MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, MSG_DIAM_EMBOLO, &planner.filament_size[active_extruder], 1.5f, 3.25f, planner.calculate_volumetric_multipliers);
            MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, MSG_DIAM_EMBOLO MSG_DIAM_E1, &planner.filament_size[0], 1.5f, 3.25f, planner.calculate_volumetric_multipliers);
            MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, MSG_DIAM_EMBOLO MSG_DIAM_E2, &planner.filament_size[1], 1.5f, 3.25f, planner.calculate_volumetric_multipliers);
            #if EXTRUDERS > 2
              MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, MSG_DIAM_EMBOLO MSG_DIAM_E3, &planner.filament_size[2], 1.5f, 3.25f, planner.calculate_volumetric_multipliers);
            #if EXTRUDERS > 3
              MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, MSG_DIAM_EMBOLO MSG_DIAM_E4, &planner.filament_size[3], 1.5f, 3.25f, planner.calculate_volumetric_multipliers);
              #if EXTRUDERS > 4
                  MENU_MULTIPLIER_ITEM_EDIT_CALLBACK(float43, MSG_DIAM_EMBOLO MSG_DIAM_E5, &planner.filament_size[4], 1.5f, 3.25f, planner.calculate_volumetric_multipliers);
                #endif // EXTRUDERS > 4
              #endif // EXTRUDERS > 3
            #endif // EXTRUDERS > 2
          #endif // EXTRUDERS > 1
	  ...
	  #if ENABLED(PASTAPRINT)  
	    /*  
		En PASTAPRINT el "porc_rend_extrusor" de husillo en boquilla afecta a los pasos por mm3.
		*/ 	
		   planner.actualiza_steps_mm3_E();
		   
		 MENU_ITEM_EDIT(float3, MSG_PASOS_MM3 "(CTCHO)", &planner.axis_steps_per_mm[E_AXIS], 1, 1000000);
		 
		#if ENABLED(EXTRUSOR_MIX)
			MENU_MULTIPLIER_ITEM_EDIT(int8, MSG_PORC_RENDMTO_EXTRUSION , &planner.factor_rendimiento_extrusion, 1, 100);
			MENU_ITEM_EDIT(float3, MSG_PASOS_MM3 "(BQLLA)", &planner.axis_steps_per_mm[E_AXIS+1], 1, 1000000);
		#endif

		#if ENABLED(OBTU_TORN)
			MENU_ITEM_EDIT(int3, MSG_PWM_OBTURADOR, &obturador_pwm_max, 1, 255);
			MENU_MULTIPLIER_ITEM_EDIT(int3, MSG_MS_OBTURADOR, &t_max_obturador_ms, 1, 500);
			MENU_MULTIPLIER_ITEM_EDIT(int3, MSG_MS_ENTRE_OPER_OBTURADOR, &t_min_entre_operaciones_obturador_ms, 0, 500);
		#endif
   ...
   */
    ...
    void lcd_control_retract_menu() {
      ...
      #endif
      MENU_ITEM_EDIT(float52, MSG_CONTROL_RETRACTF, &fwretract.retract_feedrate_mm_s, 0.05f, 999);
      MENU_ITEM_EDIT(float52sign, MSG_CONTROL_RETRACT_ZLIFT, &fwretract.retract_zlift, 0, 999);
      MENU_ITEM_EDIT(float52sign, MSG_CONTROL_RETRACT_RECOVER, &fwretract.retract_recover_length, -0.05f, 100);
      #if EXTRUDERS > 1
        MENU_ITEM_EDIT(float52sign, MSG_CONTROL_RETRACT_RECOVER_SWAP, &fwretract.swap_retract_recover_length, -0.1f, 100);
      #endif
      MENU_ITEM_EDIT(float52, MSG_CONTROL_RETRACT_RECOVERF, &fwretract.retract_recover_feedrate_mm_s, 0.05f, 999);
      #if EXTRUDERS > 1
        MENU_ITEM_EDIT(float52, MSG_CONTROL_RETRACT_RECOVER_SWAPF, &fwretract.swap_retract_recover_feedrate_mm_s, 1, 999);
      #endif
      END_MENU();
    }
 ...
 #if ENABLED(PASTAPRINT) 
    DEFINE_MENU_EDIT_TYPE(int16_t, int4, itostr4, 1);
    DEFINE_MENU_EDIT_TYPE(float, float53, ftostr43, 10);
 #endif 
