...
//#include "Marlin_main.h"
	...
	extern unsigned long liquid_posicion_upasos;
	extern bool debug2_SI;
#endif
int32_t pE0_step; 
...
float Planner::e_factor[EXTRUDERS] = ARRAY_BY_EXTRUDERS1(1.0f); // The flow percentage and volumetric multiplier combine to scale E movement
  ...
  float Planner::filament_size[EXTRUDERS],          // diameter of filament (in millimeters), typically around 1.75 or 2.85, 0 disables the volumetric calculations for the extruder
        Planner::volumetric_area_nominal = CIRCLE_AREA(float(DIAMETRO_EMBOLO_MM) * 0.5f), // Nominal cross-sectional area
        Planner::volumetric_multiplier[EXTRUDERS];  // Reciprocal of cross-sectional area of filament (in mm^2). Pre-calculated to reduce computation in the planner
#endif


 #if ENABLED(PASTAPRINT) 
 	 float   Planner::diam_boquilla;	
	 uint8_t Planner::factor_rendimiento_extrusion ;    // Porcentaje del factor de eficiencia del extrusor 
 #endif
...
#if ENABLED(AUTOTEMP)
	#if ENABLED(PASTAPRINT)
	float Planner::autotemp_max = 60,
		  Planner::autotemp_min = 10,
	#else
...
void Planner::check_axes_activity() {
   ...
   * Return 1.0 with volumetric off or a diameter of 0.0.
		Esta rutina devuelve el valor (float)=1/Seccion = volumetric_multiplier  del hilo para pasar a mm lineales  
		es decir  mm = mm3 / secccion   >>>>>  mm = mm3 * volumetric_multiplier[i]
   */
 ...
 #if ENABLED(PASTAPRINT)  
  /*  
  En PASTAPRINT no existe un filamento propiamente dicho. Se asimila a todos los efectos 
  que el diametro de filamento es igual al diametro del embolo.
  Parece que es mas claro trabajar en modo volumetrico, aunque se puede hacer en modo NO_VOLUMETRIC igualmente
  dividiendo por la seccion del embolo.
  Trabajando en modo volumetrico la cortadora proporciona en el eje E los mm3 a imprimir en cada gcode G1
  segun la (altura de capa x el ancho deseado x longitud), en lugar de mm lineales de hilo filamento. 
  Se puede convertir facilmente dado que Seccion = mm3 / mm.
  La maquina deberia tener instalada una boquilla del mismo diametro que el que se uso para el programa de slicer.
  El rendimiento de la extrusion puede no ser el 100% del teórico, para afinar el volumen extruido
  podemos modificar el "Flow".
  El "factor_rendimiento_extrusion" indica el grado de eficiencia del extrusor y se aplica
  su inverso a axis_steps_per_mm[E_AXIS]. 
  Un rendimiento del 80% significa multiplicar por 100/80=1.25 el valor calculado por la geometria de la transmisión.
  Si alguno de los valores cambia dentro de una misma sesion hay que recalcular de nuevo el <axis_steps_per_mm[E_AXIS]>.
  Esta rutina tambien recalcula position, liquid_posicion_upasos y el inverso de "axis_steps_per_mm[]" en "steps_per_mm[]" 
  Desde M16 o desde la pantalla LCD Control/Boquilla/%Rend.Extrusor. se llama a esta rutina.
  En arranque se utiliza el valor grabado en la EEPROM.
  */ 
	void Planner::actualiza_steps_mm3_E(){
		
	#if ENABLED(PRESPRINT)
		axis_steps_per_mm[E_AXIS] = 1;  //no hay motor
	#else
//		float  liquid_disponible_mm3 = liquid_posicion_upasos / planner.axis_steps_per_mm[E_AXIS];
		axis_steps_per_mm[E_AXIS] = uPASOS_VUELTA_MOTOR \
							/(CIRCLE_AREA(planner.filament_size[0]* 0.5f)* PASO_TORNILLO_MM * RELAC_MOTOR_TORNILLO ) ;
		axis_steps_per_mm[E_AXIS]*= (float)100 / planner.factor_rendimiento_extrusion ;
		planner.refresh_positioning(); // Recalcula position y steps_to_mm si axis_steps_per_mm se cambiaron!
		//recalculo la nueva posicion de upasos.
//		liquid_posicion_upasos = liquid_disponible_mm3 * planner.axis_steps_per_mm[E_AXIS];	
	#endif
	}	 
  #endif  // PASTAPRINT
 ...
 * Block until all buffered steps are executed / cleaned
   Bloquea hasta limpiar el buffer
 */
...
/**
 * Planner::_populate_block     //ESTA RUTINA ES LA QUE MUEVE TODOS LOS EJES.
 *
...
bool Planner::_populate_block(block_t * const block, bool split_move,
  ...
  //*/
    ...
    if (de) {
        ...
        if (ABS(de * e_factor[extruder]) > (int32_t)axis_steps_per_mm[E_AXIS_N] * (EXTRUDE_MAXLENGTH)) { 
		// It's not important to get max. extrusion length in a precision < 1mm, so save some cycles and cast to int
          if (COUNT_MOVE) {
            position[E_AXIS] = target[E_AXIS]; // Behave as if the move really took place, but ignore E part
            #if HAS_POSITION_FLOAT
              position_float[E_AXIS] = target_float[E_AXIS];
  ...
  // For a mixing extruder, get a magnified esteps for each
	...
	if (esteps) SERIAL_ECHO("\t(up)");
    for (uint8_t i = 0; i < MIXING_STEPPERS; i++){
		 block->mix_steps[i] = mixing_factor[i] * esteps;
		if (esteps){
			SERIAL_ECHOPAIR(" E", i);
			SERIAL_ECHOPAIR(":", block->mix_steps[i]);
			SERIAL_ECHO("\t");
		}
	}
	if (esteps) SERIAL_ECHO("\n");
  #endif
  ...
  if (esteps)
    NOLESS(fr_mm_s, min_feedrate_mm_s);    // sol ambos 0.0
  else
...
/*	
	* Calcule la velocidad de entrada máxima permitida en el cruce por aproximación de aceleración centrípeta.
     Deje que un círculo sea tangente a los segmentos de línea de ruta anterior y actual, 
	 donde la desviación del cruce se define como la distancia desde el cruce hasta el borde
	 más cercano del círculo, colineal con el centro del círculo. 
	 El segmento circular que une los dos caminos representa el camino de la aceleración centrípeta. 
	 Resuelva la velocidad máxima en función del máximo sobre el radio del círculo, definido 
	 indirectamente por la desviación de la unión. Esto también se puede ver como ancho de ruta
	 o max_jerk en la versión anterior de Grbl. Este enfoque en realidad no se desvía de la ruta,
	 pero se usa como una forma robusta de calcular las velocidades en las curvas, ya que tiene 
	 en cuenta las no linealidades tanto del ángulo de la unión como de la velocidad de la unión.
     *
     * NOTA: Si el valor de desviación del cruce es finito, Grbl ejecuta los movimientos en un modo
	 de trayectoria exacta (G61). Si el valor de desviación del cruce es cero, Grbl ejecutará el 
	 movimiento en un modo de parada exacta (G61.1). En el futuro, si se desea el modo continuo (G64),
	 las matemáticas aquí son exactamente las mismas. En lugar de moverse hasta el punto de unión,
	 la máquina simplemente seguirá el círculo del arco definido aquí. 
	 El Arduino no tiene los ciclos de CPU para realizar una ruta de modo continuo, pero los 
	 microcontroladores basados ​​en ARM ciertamente los tienen.
     *
     * NOTA: La velocidad máxima de cruce es un valor fijo, ya que los límites de aceleración de
	 la máquina no se pueden cambiar dinámicamente durante la operación ni la línea puede mover 
	 la geometría. Esto debe guardarse en la memoria en caso de que una anulación de la velocidad
	 de avance cambie las velocidades nominales de los bloques, lo que puede cambiar las condiciones
	 generales de velocidad de entrada máxima de todos los bloques.
     *
     * #######
     * https://github.com/MarlinFirmware/Marlin/issues/10341#issuecomment-388191754
     *
     * hoffbaked: el 10 de mayo de 2018 ajustó y mejoró el algoritmo GRBL para Marlin:
          ¡De acuerdo! Parece estar funcionando bien. Lo corté arbitrariamente a 1 mm y luego 
		  en cualquier cosa con menos lados que un octágono. Con esto, y el paso inverso realmente
		  recalculando cosas, un valor de aceleración de esquina de 1000 desviación de cruce de .05 
		  es bastante razonable. Si se pueden ahorrar los ciclos, se podría usar un mejor acos. 
		  Por lo que sé, es posible que ya esté calculado en un lugar diferente. 
 */
    // Vector unitario del segmento de línea de ruta anterior
...
bool Planner::buffer_segment(const float &a, const float &b, const float &c
  ...
  const int32_t target[NUM_AXIS] = {
    #if ENABLED(LINE_BUILDUP_COMPENSATION_FEATURE)	//No para UVPRINT
      ...
      #endif
    #endif
    LROUND(e * axis_steps_per_mm[E_AXIS_N])   //IMPORTANTE: "e" y "axis_steps_per_mm[3]" deben estar en las mismas unidades 
											  //en nuestro caso en mm3 y pasos/mm3.
  };

  #if HAS_POSITION_FLOAT				//false para PASTAPRINT
...
/* <-- add a slash to enable
 ...
 //   SERIAL_ECHOLNPGM(")");
	    SERIAL_ECHOPGM(")");
    SERIAL_ECHOLNPAIR(">>>", axis_steps_per_mm[E_AXIS]);
...
// !!! Antes de entregar el movimiento a stepper insertamos esto:
// Abre obturador si:
//   target[E_AXIS] > pE0_step  siendo pE0_step la posicion de target[E_AXIS] anterior.           ]
//   En caso contrario cierra el obturador
 
//if (IsRunning())
 //SERIAL_ECHOPAIR("\t upasos=",target[E_AXIS]-pE0_step);
 //SERIAL_ECHO(E0_DIR_PIN);
if (target[E_AXIS] > pE0_step){
	// hay pasta que imprimir
	if (READ(FC_OPEN_OBT_PIN) == HIGH){  //si esta cerrado el obturador FC esta en HIGH
		abre_obturador();
	}
}else{
	// Solo es movimiento
	if (READ(FC_OPEN_OBT_PIN)== LOW){ //si esta abierto el obturador FC esta en LOW
		cierra_obturador();
	}
}
pE0_step = target[E_AXIS];
