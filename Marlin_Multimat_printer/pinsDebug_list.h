//	**********  MODIFICADO para MULTIMAT_PRINTER  Abril-20  ***********
/**   pinsDebug_list.h
...
REPORT_NAME_ANALOG(__LINE__, FILWIDTH_PIN)
  ...
  REPORT_NAME_ANALOG(__LINE__, ADC_KEYPAD_PIN)
	...
	#if  ENABLED(PASTAPRINT_DETECTA_AUTO_PASTA)
		REPORT_NAME_DIGITAL(__LINE__, SENSORES_PASTA_PIN)   //$$ Añadido	//morado
		REPORT_NAME_DIGITAL(__LINE__, HAY_PASTA_PIN)       	//$$ Añadido	//blanco
	#endif
	#if ENABLED(PRESPRINT)
		 REPORT_NAME_DIGITAL(__LINE__, START_PIN)			
		 REPORT_NAME_DIGITAL(__LINE__, PRESION_PIN)			
	#endif
	#if  ENABLED( PASTAPRINT_EXISTE_PRESOSTATO)
		REPORT_NAME_DIGITAL(__LINE__, PRESOSTATO_PIN)		//verde		
	#endif
#endif


#if  ENABLED(UVPRINT)
	 REPORT_NAME_DIGITAL(__LINE__, RELES_UV_PIN) 

// REPORT_NAME_DIGITAL(__LINE__, FC_EXTR_TOP_PIN)
 #endif

#if ENABLED(EMBOLO_TORNILLO)	
	 REPORT_NAME_DIGITAL(__LINE__, FC_MORDAZA_ACCESIBLE_PIN) 
	 REPORT_NAME_DIGITAL(__LINE__, LIBERAR_MORDAZA_PIN)
#endif 
		 
#if ENABLED(PASTAPRINT)
	 REPORT_NAME_DIGITAL(__LINE__, BAJAR_CAMA_PIN)      		
	 REPORT_NAME_DIGITAL(__LINE__, SENTIDO_AVANCE_MANUAL_PIN)
	 REPORT_NAME_DIGITAL(__LINE__, SANGRADO_MANUAL_PIN)	 

 #if ENABLED(OBTU_TORN)
	 REPORT_NAME_DIGITAL(__LINE__, PWM_OBTURADOR_PIN) 	// 65 PWM para el obturador
	 REPORT_NAME_DIGITAL(__LINE__, HALF_PIN )  			//  59  Obturador FC
	 REPORT_NAME_DIGITAL(__LINE__, OPEN_PIN)     		// 64 Obturador FC
	 REPORT_NAME_DIGITAL(__LINE__, IN1_PIN)				// 4 Obturador direccion
	 REPORT_NAME_DIGITAL(__LINE__, IN2_PIN) 		 	// 5 Obturador direccion
#endif
#if ENABLED(OBTU_SOLE) 
	REPORT_NAME_DIGITAL(__LINE__, PWM_OBTURADOR_PIN) 	// 65 PWM para el obturador
	REPORT_NAME_DIGITAL(__LINE__,  FC_OPEN_OBT_PIN )   	// FC Obturador normalmente abierto= HIGH  cerrado=LOW
#endif
