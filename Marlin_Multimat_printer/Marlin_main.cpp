/**********  Marlin MODIFICADO para MULTIMAT_PRINTER  Junio-21  ***********
/**
...
*/
#include   <U8glib.h>   // Necesaria para el RepRapDiscount
#include "Marlin.h"
...
//Variables globales de PASTAPRINT
//bool sw_recargando_pasta= false;   //indicador para bajar la velocidad de impresion hasta recuperar el suministro de pasta
float sangrado_mm3_min = MM3_MIN_SANGRADO; //Se modifica con gcode M13 para la sesion. (rpm para el sangrado manual)

#if  ENABLED(LIQUID)
float recarga_mm3=0;		//se emplea en planner.cpp en (LIQUID)modo
#endif

// Mensajes de debug para el HOST. Utilidad incorporada para UVPRINT
bool debug_SI = true;			//=true envia mensajes a HOST   =false no envia  
bool debug2_SI = true;			//=true envia mensajes a HOST   =false no envia  

 //variables globales de DEBUG_NO_REP
const char * msj_anterior; 
unsigned int msj_fin=0;

 void prueba_cierra_obturador(){
	 millis_t inicia;
	 for (float v=10;v<15.5;v+=0.5){
		 SERIAL_ECHO_F(v,2);
	 SERIAL_ECHOPAIR_F("V\t pwm= ",v * 25);		 
		 inicia=millis();
	 analogWrite(PWM_OBTURADOR_PIN, v * 25);
	 while (ESTA_OPEN){ };
	 int tmp= (int)(millis()-inicia);
	 SERIAL_ECHOLNPAIR("\t\t",tmp);
	 
	 analogWrite(PWM_OBTURADOR_PIN, 0);	 
	 delay(1000);
	 }
SERIAL_ECHOLN(" ");
 }
	...
	int obturador_pwm_max = OBT_PWM_MAX;  //valor pwm para el L298N del motor del obturador (0 a 255)
	int t_max_obturador_ms = MS_LIMITE_OBTURADOR;  //tiempo maximo de operacion HALF del obturador en milisegundos 
	int t_min_entre_operaciones_obturador_ms = MS_MIN_ENTRE_OPER_OBTURADOR;  //Tiempo en miliseg minimo entre operaciones			
#endif


#if ENABLED(UVPRINT)
extern millis_t UV_time = 0;    //control de tiempo de encendido y apagado UV 
#endif

float E_anterior_mm3 = 0;			// auxiliar linea anterior. Se emplea en gcode_G1
E_modo G1_E_modo = INICIO;			//Accion del gcode G1. 0=INICIO 1=EXTRUSION -1=RETRACCION 2=MOVIMIENTO
bool Retractado = false;
...
#if ENABLED(UVPASTAPRINT)
float acumulado_mm3 = 0;

#endif

#if ENABLED(OBTU_SOLE)
//variables globales 
//Esta variable es por seguridad para que no se queme
	millis_t obturador_time_fin_ms = 0; // maximo tiempo en segundos que puede estar conectado el electroiman
...
float filament_width_nominal = DIAMETRO_EMBOLO_MM,  // Nominal filament width. Change with M404.
...
float mixing_factor[MIXING_STEPPERS]={1.00,1.00}; // Reciprocal of mix proportion. 0.0 = off, otherwise >= 1.0.
...
void inicia_PASTAPRINT(){

	G1_E_modo = INICIO;
    E_anterior_mm3 = 0;
	
	#if ENABLED(OBTU_TORN)
		half_open_obturador(obturador_pwm_max, 1);   //inicio half el obturador. Accion 1 
	#endif
		...
		if HAY_PRESION SERIAL_ECHOLN(" Hay presión");	
		else SERIAL_ECHOLN(" No hay presion");
	#endif
...
#if ENABLED(UVPRINT)
/************************* reset_UVPRINT ************************************
Reseteo todos los dispositivos apagando luces
*/
void reset_UVPRINT(){
	HAY_PASTA = false;
		//desactivo reles de UV  K4=UV_izda y K5=UV_dcha (conmutan 12 V)
	apagar_UV();

	SERIAL_PROTOCOLLN(MSG_RESET_UVPRINT);
	G1_E_modo = INICIO;
    E_anterior_mm3 = 0;


//	WRITE(UV_IZDA_CTROL_PIN,LOW);
//	WRITE(UV_DCHA_CTROL_PIN,LOW);
}
...
void init_UVPRINT(){
	HAY_PASTA = false;
		//desactivo rele de UV  K4 (conmuta 12 V alimentan los 2 reguladores de 3.4 V)

	apagar_UV();

	SERIAL_PROTOCOLLN(MSG_INIT_UVPRINT);
	G1_E_modo = INICIO;
    E_anterior_mm3 = 0;
...
/************************* encender_UV ************************************
Enciendo luces UV y sus ventiladores
*/
void encender_UV(){
	if (!UV_ACTIVADO) {    //Si no esta activado....
		planner.synchronize();
		ACTIVAR_UV;
		UV_time = millis();
		if (debug2_SI) SERIAL_PROTOCOL(MSG_UV_ACTIVADO "\t");
	}
}

/************************* apagar_UV ************************************
Apago luces UV y sus ventiladores
*/
void apagar_UV(){
	if (UV_ACTIVADO) {
		planner.synchronize();
		DESACTIVAR_UV;
		if (debug2_SI) SERIAL_PROTOCOL(MSG_UV_APAGADO "\t");
	}
}
#endif //UVPRINT

#if ENABLED(PASTAPRINT) && ENABLED(EXTRUSOR_MIX)	//Version con dos extrusores con motor uno de embolo y otro en boquilla

/*
En PASTAPRINT coexisten para el eje E, dos extrusores. Uno para la extraccion de la pasta del 
cartucho y otro que actua como dosificador en la boquilla, ambos están colocados en serie y deben 
suministrar la misma cantidad de mm3/segundo de pasta para mantener estable la presion en la boquilla, 
por eso hay que calibrar fisicamente esta condición.

Marlin dispone de la opción MIXING_EXTRUDER que puede usarse para manejar favorablemente esta 
configuracion de dos extrusores en serie a traves de la variable de reparto mixing_factor[], 
que aqui la usamos como variable de ratio entre las velocidades pasos/mm3 de ambos extrusores. 

El cartucho funciona mediante embolo y por tanto se supone que entrega muy aproximadamente la 
cantidad calculada (Rendimiento=100%) pero en la boquilla por sus caracteristicas de arrastre
helice sin-fin, interviene la compresion del material, limpieza, presion, etc, y 
solo se puede ajustar mediante ensayos, que nos dan el parametro de rendimiento real de 
la extrusion en la boquilla. 

Para igualar el flujo de pasta, ambos deben entregar pasta en cada momento a la misma velocidad 
en mm3/seg, pero cada motor da un numero de pasos diferente en funcion de su relacion de transmision 
y su caracteristica intrinseca de pasos/mm3. 

Para conseguir este sincronismo se propone lo siguiente:

	* Considerar como extrusor principal el del Cartucho, dado que le hemos supuesto un 100% de efectividad 
		por lo que los pasos calculados para ese extrusor corresponden a la extraccion de pasta deseada. 
		Esto significa que el eje E_AXIS debe referise al Cartucho y no a la Boquilla, por tanto su mixing_factor[]
		es el que corresponde al indice [0].
	* Asignamos al Cartucho un mixing_factor[0]=1.
	* Asignamos al motor de la Boquilla  mixing_factor[1] = BQLLA_pasos_mm3 / CTCHO_pasos_mm3.
*/
void extrusor_mix_pastaprint(){
	mixing_factor[0]= 1.00;      								//correspondiente al Extrusor CARTUCHO
	mixing_factor[1]= BQLLA_PASOS_MM3_REALES / CTCHO_PASOS_MM3 ;  //correspondiente al extrusor BOQUILLA
}
#endif
	...
	/* ******************************************retractar_1()****************************
	 Retractar_1 se usa para obturadores con accionamiento de tornillo.
	Realiza una espansion del conducto de silicona que suministra pasta a la boquilla.
	 Se ha observado que el sistema de leva con roldana empleado produce un aumento del volumen disponile 
	 en el interior y en consecuencia produce un efecto de succion (retracción) en la boquilla.
	 Naturalmente para eso la posicion de partida debe ser la de "close", siendo esta la posicion en que 
	 debe permanecer en el tiempo de extrusion normal, con lo cual la posicion que denominamos "close" esta 
	 en realidad  entreabierto.
	 Por todo ello la posicion normal es la de HALF.
	 Para evitar retracciones duplicadas se ha creado la variable global bool "Retractado" true=SI  false=NO
	 Anteriormente.....(La longitud a retractar RETRACT_LENGTH(mm hilo) y la velocidad RETRACT_FEEDRATE(mm hilo/seg)
	 ver valores en Configuration_adv.h)
	 Ahora se ordena al obturador el HALF
	 */
void retractar_1(){
	if (Retractado == true) return;
		#if ENABLED(OBTU_TORN)	
		//Calculo el tiempo necesario para el viaje para apagar o no UV
		if (gcode_time_next_move() <  t_min_entre_operaciones_obturador_ms){
			SERIAL_PROTOCOLPAIR("time No Retracta=", gcode_time_next_move() );
			return;
		}else{
			SERIAL_PROTOCOLPAIR("time Retracta=", gcode_time_next_move() );
				half_open_obturador(obturador_pwm_max, -1);  //Accion 1 paso a OPEN para ASPIRAR pasta de la boquilla abriendo obtturador
			}
		#endif
		#if ENABLED(FWRETRACT)
...
/* recuperar() realiza una recuperacion volumentrica igual al volumen retractado 
*/
void recuperar(){
	if (Retractado == false) return;
	
#if ENABLED(OBTU_TORN)
    half_open_obturador(obturador_pwm_max, 1);   //Accion 1 para RECUPERAR pasta paso a HALF.
#endif
	
#if ENABLED(OBTU_SOLE)
	cierra_obturador(); // Disable electroiman
#endif
...
/************************* stop_mensaje ************************************
Mensaje al display LCD con parada durante <tseg> segundos.
 Si tseg=0  Espera a la pulsacion del usuario con tiempo indefinido. La salida es <false>
 Si tseg>0  Si se pulsa antes del tiempo la salida es <false>
			Si se consume el tiempo fijado sin pulsar la salida es <true>.

*/
bool stop_mensaje(const char * mesg, float tseg){
lcd_reset_alert_level();  //pongo el nivel a cero
lcd_setstatus(mesg);
...
sonidos(1);
if (tseg > 0) {
	millis_t ms = tseg * 1000L + millis(); // wait until this time for a click
    while (PENDING(millis(), ms) && wait_for_user) {idle();}
	if (millis() >= ms) salida = true;
}
else
...
//
/************************** char tex_y_num *******************************************
Devuelve una cadena de texto <buffer> concatenado con un <valor> de <dim> char y <dec> decimales
*/
const char * tex_y_num(char * buffer, float valor, int dim, int dec){
char bufn[9]; //Cadena donde almacenaremos el número convertido
char linea[22];
strcpy(linea,buffer);
buffer=NULL;
return strcat(linea,dtostrf(valor, dim, dec, bufn));
}

/*
********************  DEBUG_NO_REP *****************************
Si la global <debug_SI> es true visualiza el mensaje en pantalla si no_repetir=false.
Si no_repetir=true si el mensaje es igual que el ultimo usado por la rutina imprime (.)
Cada 100 (.) cambia de linea.
*/
void DEBUG_NO_REP(const char * msj, bool no_repetir=false){
	if (!debug_SI) return;
	if (no_repetir) {
		if (msj == msj_anterior){
			msj_fin ++;
			if (msj_fin >= 50) {SERIAL_PROTOCOL(" "); msj_fin=1;}
			(msj_fin % 10 == 0) ? SERIAL_PROTOCOL("0"): SERIAL_PROTOCOL(".");
			return;
		}else{
			if (msj_fin > 1) SERIAL_PROTOCOL(" ");
			SERIAL_PROTOCOL(msj);
			SERIAL_PROTOCOL("(.)");
			msj_anterior = msj;  //guardo el puntero msj
			msj_fin = 1;
		}
	}else{
		if (msj_fin > 0) SERIAL_PROTOCOL("\n");
		SERIAL_PROTOCOL(msj);
//		msj_fin=0;
	}
}
...
/*************** sonidos **************************************
 ...
 Frequencia (Hz) de notas de octava cero.
 Calculamos las frecuencias de otras octavas a partir de esta para ahorrar espacio .
 Para obtener frecuencias en otras octavas fr(octava)= fr * (2**(octava-cero))
 por ejemplo octava 4ª seria fr_4 = fr * (2*2*2*2)
 PARA AHORRAR ESPACIO las escalas se dan como tipo integer para dividir por 100.
*/

//definimos la duración: doble, entera, media y cuarta 	(en milisegundos)
#define Dd  1000
#define De  500
#define Dm  250
#define Dc  125
//definimos la frecuencia de las notas en la octava 0
#define Do  16.35  
#define DoS  17.32
#define Re   18.35
#define ReS  19.45
...
/************************************ sonidos *********************
 sonidos para el zumbador de la LCD
Datos de entrada:
 		secuencia: Numero de secuencia musical (tiene de 1 a 4 notas)
 swecuencias:
	0:	aviso de inicio o atencion
	1:	aviso de carga de hilo
	2:	aviso de error
	3:	bajada de cama
	9:	aviso de finalizacion
*/
...
void sonidos(unsigned char secuencia){
	...
	int fr;
	switch (secuencia){    
			case 0:		//aviso de inicio o atencion. Con tiempo minimo de notas "c"
		{		
			uint16_t	duracion[]	={Dd,Dd,Dd	,0};
			float 		escala[]	={Do,Re	,Mi	,0};
			uint8_t 	octava[]	={1	,1	,1	,0};    
		for (int i=0;i<4;i++){dur[i]=duracion[i];esc[i]=escala[i];oct[i]=octava[i];}	
		}
		break;

		case 1:		//aviso de carga de hilo
		{		
			uint16_t 	duracion[]	={Dd,Dd	,Dd	,Dd};
			float 		escala[]	={Mi,ReS,Si	,Mi};
			uint8_t 	octava[]	={2,2,2,2};	
		for (int i=0;i<4;i++){dur[i]=duracion[i];esc[i]=escala[i];oct[i]=octava[i];}	
		}
		break;

		case 2:		//aviso de error en carga
		{			
			uint16_t  	duracion[]	={Dd,Dd,Dd,Dd};
			float 		escala[]	={Do,Re,Mi,Do};
			uint8_t 	octava[]	={0,0,0,1};
		for (int i=0;i<4;i++){dur[i]=duracion[i];esc[i]=escala[i];oct[i]=octava[i];}	
		}
		break;                   

		case 3:		//bajada de cama
		{		
			uint16_t  	duracion[]	={Dc,Dm,Dc,0};
			float 		escala[]	={Mi,Do,Mi,0};
			uint8_t 	octava[]	={1,4,1,0};	
		for (int i=0;i<4;i++){dur[i]=duracion[i];esc[i]=escala[i];oct[i]=octava[i];}	
		}	
		break;
		
		case 4:		//Ventana acceso a tuerca partida
		{		
			uint16_t  	duracion[]	={Dd,0,0,0};
			float 		escala[]	={Mi,0,0,0};
			uint8_t 	octava[]	={4,0,0,0};	
		for (int i=0;i<4;i++){dur[i]=duracion[i];esc[i]=escala[i];oct[i]=octava[i];}	
		}	
		break;

		case 9:		//aviso de finalizacion
		{			
			uint16_t  	duracion[]	={Dd,0,0,0};
			float 		escala[]	={FaS,0,0,0};
			uint8_t 	octava[]	={0,0,0,0};	
		for (int i=0;i<4;i++){dur[i]=duracion[i];esc[i]=escala[i];oct[i]=octava[i];}	
		}		
		break;
		...
		{		
			uint16_t	duracion[]	={Dd,Dd,Dd,Dd};
			float 		escala[]	={Do,Re,Mi,FaS};
			uint8_t 	octava[]	={0	,0	,0	,0};    
		for (int i=0;i<4;i++){dur[i]=duracion[i];esc[i]=escala[i];oct[i]=octava[i];}	
		}
		break;		
}		
		for (uint8_t i = 0; i < 4; i++){  // suena la secuencia
		    if (dur[i]>0) {
				fr = (float) esc[i] * pow(2.0,(float)oct[i]);
				BUZZ(dur[i],fr);
			}
		}
//		SERIAL_ECHOPAIR("DURA:",buff[i].duracion);
//		SERIAL_ECHOLNPAIR(" FREC:",fr[]
 noTone(BEEPER_PIN);
}
...
/*	
	if HAY_PRESION return;
	SERIAL_ECHO("Activando presion .");
	millis_t  ahora = millis()+10000;
//	while (!HAY_PRESION && ahora > millis()) {
//		SERIAL_ECHO(".");
		delay(5000);
//	}
	...
	SERIAL_ECHO(":");	
	if HAY_PRESION {
		 SERIAL_ECHOLN(" HAY PRESION");
	}else{
		WRITE(START_PIN, LOW);
		delay(2);
	}
		SERIAL_ECHO(READ(START_PIN));
		SERIAL_ECHO(READ(PRESION_PIN));
*/		
}
...
/*
	SERIAL_ECHO("Desactivando presion .");
	millis_t  ahora = millis() + 5000;
	while (HAY_PRESION && ahora < millis()){
		SERIAL_ECHO(".");
		delay(250);
	}		
	if (!HAY_PRESION) SERIAL_ECHOLN(" OK");
*/
}
#endif  //PRESPRINT
...
/*
/*************************** operar_ValvLimpieza (int nservo, int operacion) **********************
 Maneja los dos servos de la valvula de limpieza modelo SPT5435LV-180W  de 35K de fuerza maxima
 la libreria servo.h se ha modificado para este modelo que trabaja con frecuencia minima de 330 Hz
 y con un rango de 500 a 2500 microsegundos  para 180º y frecuencia posicion central 1500.
 Entradas: (int) nservo    (0=DEPOSITO)  (1=LIMPIEZA)
		 (int) posiciones 	0 = (0º) = Aire a salida
							1 = (45º) = bloqueo, no hay conexion de entrada con la salida
							2 = (90º) = Agua a salida) , pero
La alimentacion +5V de los servos se hace a traves del Rele K7 que es manejado por el pin SERVOS_PIN
*/
void operar_ValvLimpieza (int nservo, int mover){
int ang=0; // por defecto ang estado de Bloqueo
//char buf[22]={0};
ACTIVAR_ALIMENTACION_SERVOS;   //Activo rele K7 conecto +5V a los servos
delay(50);
switch (mover) {
    case 0 : ang = 1500; break;	 //aire
	case 1 : ang = 2000; break;  //bloqueo
    case 2 : ang = 2500; break;	 //agua
	default : ang=mover; break;
}
switch (nservo) {
	case DEPOSITO :
			servo[DEPOSITO].attach(SERVO0_PIN,500,2500);
			servo[DEPOSITO].writeMicroseconds(ang);
			delay(50);
			if (debug_SI) SERIAL_PROTOCOLPGM("DEPOSITO. ");
//			strcpy(buf,"DEPOSITO. ");
			servo[DEPOSITO].writeMicroseconds(ang);
			break;

	case LIMPIEZA :
			servo[LIMPIEZA].attach(SERVO1_PIN,500,2500);
			servo[LIMPIEZA].writeMicroseconds(ang);
			delay(50);
			if (debug_SI) SERIAL_PROTOCOLPGM("LIMPIEZA. ");
//			strcpy(buf,"LIMPIEZA. ");
			servo[LIMPIEZA].writeMicroseconds(ang);
			break;
	default : return;
...
if (debug_SI){
	switch  (mover) {
		case 0 : SERIAL_PROTOCOLLNPGM("aire");
				break;
		case 1 : SERIAL_PROTOCOLLNPGM("bloq");
				break;
		case 2 : SERIAL_PROTOCOLLNPGM("agua");
				break;
	}
}
//DEBUG_NO_REP(buf,false);
			delay(1000);
			servo[nservo].detach();
DESACTIVAR_ALIMENTACION_SERVOS;	//Desactivo rele K7 de corriente
}
#endif  //PASTAPRINT_LIMPIEZA


#if  ENABLED(PASTAPRINT_LIMPIEZA)

/*************************************** parar_BombaAgua() **********************
 Rutina para el paro de la bomba de agua de limpieza.
pasa a high el estado del pin BOMBA_AGUA_PIN  y espera a que se vacien los conductos para proteger la valvula de limpieza.
*/
void parar_BombaAgua(){
	WRITE(BOMBA_AGUA_PIN, HIGH);

	DEBUG_NO_REP(MSG_BOMBA_AGUA ": parada",true);
//	SERIAL_ECHOLNPGM(MSG_BOMBA_AGUA ": parada");
	delay(LIMPIEZA_TIEMPO_DESCARGA_AGUA_ms);
}
/*************************************** activar_BombaAgua() **********************
 Rutina para poner en marcha la bomba de agua de limpieza.
Primero comprueba la posicion de las valvulas de limpieza.
Para poner en marcha la bomba debe estar al menos una en posicion de "Agua"
*/
void activar_BombaAgua(){
int pos0, pos1;
	servo[0].attach(SERVO0_PIN,500,2500);
	servo[1].attach(SERVO1_PIN,500,2500);
	pos0=servo[0].read();  //lee la posicion del servo
//	SERIAL_ECHOLN(pos0);
	pos1=servo[1].read();
//	SERIAL_ECHOLN(pos1);
	servo[0].detach();
	servo[1].detach();
	if (pos0==180||pos1==180) {
		WRITE(BOMBA_AGUA_PIN, LOW);
		SERIAL_ECHOLNPGM(MSG_BOMBA_AGUA ": activada");
	} else {
		WRITE(BOMBA_AGUA_PIN, HIGH); 	// La paro por seguridad
		SERIAL_ECHOLNPGM(MSG_BOMBA_AGUA "!No se activó!");
	}
}


/*************************************************** posicion_limpieza() **********************
 Rutina para el posicionamiento del carro y bajada de la cama para la limpieza.
*/
void posicion_limpieza(){

// primero se ha de haber hecho homing del eje Z
if ( !axis_unhomed_error(parser.seen('X'), parser.seen('Y'), parser.seen('Z'))){home_all_axes();}
	planner.synchronize();
	point_t park_point = NOZZLE_PARK_POINT;
  // Move XY to starting position, then Z
	do_blocking_move_to_xy(park_point.x, park_point.y, NOZZLE_PARK_XY_FEEDRATE);

   		//sonido de aviso de bajada de cama=3
     sonidos(3);
  // Set Z_AXIS to saved position
	do_blocking_move_to_z(park_point.z, NOZZLE_PARK_Z_FEEDRATE);

	gcode_M18_M84();   //disable steppers
	// aviso para colocacion sumidero
	      sonidos(1);
}
//********************************************* limpieza_inicial *********************
// Esta rutina expulsa la pasta residual hasta cumplirse el tiempo establecido o hasta que en el nivel 1 no haya pasta
// Cada decima parte del tiempo establecido comprueba si ha finalizado.
void limpieza_inicial() {

	// Si no hay pasta no hay nada que sacar
	if (!HAY_PASTA) return;

	SERIAL_ECHOLNPGM(MSG_LIMPIEZA_INICIAL);
	stop_mensaje(MSG_LIMPIEZA_INICIAL,1);


	float t_giro;
	long t_limite=millis()+1000L* LIMPIEZA_TIEMPO_EXPULSION_RESTOS_s;
	bool act=false;

	operar_ValvLimpieza (DEPOSITO, Aire);   		// DEPOSITO a Aire
 	operar_ValvLimpieza (LIMPIEZA, Aire);	  // LIMPIEZA   a Aire
	activar_Compresor();

	// Comprobare si he finalizado en cada fracción del tiempo disponible (10 fracciones)
	t_giro = (float)(LIMPIEZA_TIEMPO_EXPULSION_RESTOS_s / 10 *1000L) ;   //segundos por fraccion
	float vueltas = (LIMPIEZA_RPM_HUSILLO+200)*t_giro;
	
    while ( millis() < t_limite && HAY_PASTA) {
		girar_tornillo(vueltas, LIMPIEZA_RPM_HUSILLO+200, 1);  //solo en avance (ANTICLOCKWISE)
	}
	parar_Compresor();
}
...
//*************************************************** limpieza.cpp **********************
/*
  Subprograma para efectuar la limpieza del extrusor y conductos.
  El CICLO consiste en introducir agua y aire alternativamente por los conductos del cuerpo extrusor
  y, a la vez mover el tornillo adelante y atras para mejorar el arrastre del material (pasta).
  El cambio de aire/agua se maneja mediante los servos de la la valvula de limpieza compuesta por dos
  valvulas independientes pero con una entrada comun para el agua, (SERVO[0] para el conducto de presion del deposito
  y (SERVO 1) que se aplica directamente a los conductos del extrusor.
  Los servos son del tipo MG995 operados desde Marlin en la ATMega2560/RAMPS 1.4. y
  disponen de tres posiciones: Angulo 0º = Aire a Salida    Angulo 45º= Todas las vias cerradas   Angulo 90º= Agua a Salida
  En la oiperacion de limpieza la valvula DEPOSITO se coloca en bloqueo, posicion "Bloq" y la de LIMPIEZA la maneja
  el programa dando paso al agua y al aire segun una secuencia establecida, ensayada para el mejor resultado, duracion del giro
  del tornillo, velocidad, etc.,
  Las tres posiciones permiten el bloqueo de retornos para impedir que se ensucien los conductos de aire por debajo de esta valvula.

  Los pines, parametros y variables que usa este programa son:
			pines
  //Actuadores de rele
#define  SERVOS_PIN      			59			//marron    (OUTPUT) Da y corta la corriente a los servos cuando estan en la posicion. Activa el rele K7 que conmuta 5V DC
#define  SENSORES_PASTA_PIN      	63			//morado	(OUTPUT) Controla la lectura de los sensores de pasta. Activa el rele K6 que conmuta 5V DC
#define VALV_DESFOGUE_PIN  	        65			//naranja	valvula de desfogue del circuito de presion. Activa el rele K5. Conmuta 12V DC
#define BOMBA_AGUA_PIN				66			//gris		Bomba de agua. Activa rele K3. Conmuta 12V DC

  NOZZLE_PARK_POINT { (X_MAX_POS/2), (Y_MAX_POS/2), (Z_MAX_POS) }
  NOZZLE_PARK_XY_FEEDRATE 100   					// X and Y axes feedrate in mm/s
  NOZZLE_PARK_Z_FEEDRATE 5      					// Z axis feedrate in mm/s

  La secuencia de acciones de limpieza son:
  	 Retirar el cartucho de pasta, por otro vacio, si se desea.
	 Bajar  la cama y poner el extrusor en el centro a la posicion NOZZLE_PARK_POINT
	 Situar la valvula de DEPOSITO en posicion Bloqueo.
	 Se realiza una limpieza inicial de restos de pasta con presion+giro tornillo

  Repetir el ciclo de limpieza LIMPIEZA_NUMERO_CICLOS veces.
	1-		Situar la valvula de LIMPIEZA en posicion Agua. Activar la bomba de agua.
	2- 		Situar la valvula de LIMPIEZA en posicion Aire. Activar el compresor.

	Por ultimo soplar para secar el interior de conductos y tornillo

_____Manejo de los servos de la valvula de limpieza
	Entradas: (int) nservo  (0= servo DEPOSITO   1= servo LIMPIEZA)
			  (int) operacion 	0 = (0º)  Aire a salida;
								1 = (45º) Entrada y salida bloqueadas;
								2 = (90º) Agua a salida) ,
*/
...
void limpieza(){
    ...
    float  t_operacion,  t_giro;

	(HAY_PASTA) ? t_total = LIMPIEZA_TIEMPO_EXPULSION_RESTOS_s : t_total=0;
	t_total+= LIMPIEZA_TIEMPO_CICLO_s * LIMPIEZA_NUMERO_CICLOS;

	t_total+= LIMPIEZA_TIEMPO_SECADO_FINAL_s;

	SERIAL_ECHOPAIR(MSG_TIEMPO_LIMPIEZA, t_total);
	SERIAL_ECHOLNPGM(_SEG);

	if (stop_mensaje(tex_y_num("<>" MSG_TIEMPO_LIMPIEZA,t_total, 3, 0),3)) {
		posicion_limpieza();
		planner.synchronize();
	}
	parser.string_arg = MSG_SUMIDEROS;
	gcode_M0_M1();    // parada incondicional

	limpieza_inicial();

	operar_ValvLimpieza (DEPOSITO, Bloq);

  for (int x = 0; x < LIMPIEZA_NUMERO_CICLOS; x++) {

// ___________INICIO SECUENCIA ________________________________________________________
	stop_mensaje(tex_y_num(MSG_LIMPIEZA_CICLO,x+1,2,0),1);
 	SERIAL_ECHOLNPAIR(MSG_LIMPIEZA_CICLO, x+1);
	parar_BombaAgua();
	parar_Compresor();

	// -------------------- inicio operacion agua

 	operar_ValvLimpieza (LIMPIEZA, Agua);
	activar_BombaAgua();  // pin 66 a LOW

	t_operacion = (float)(LIMPIEZA_TIEMPO_CICLO_s * (100 - LIMPIEZA_RATIO_TIEMPO_AIRE ) / 100L); 	//segundos de agua
	giro_alternado_tornillo(t_operacion, LIMPIEZA_RPM_HUSILLO);
	parar_BombaAgua();

	// --------------------- inicio operacion aire.
   	operar_ValvLimpieza (LIMPIEZA, Aire);
	activar_Compresor();

    t_operacion = (LIMPIEZA_TIEMPO_CICLO_s * LIMPIEZA_RATIO_TIEMPO_AIRE )/100L ; 	//segundos de aire
	giro_alternado_tornillo(t_operacion, LIMPIEZA_RPM_HUSILLO);

	// ___________FIN SECUENCIA ________________________________________________________
  }

  // secado final. Envio aire por ambos conductos.
  	stop_mensaje(MSG_SECADO, 1);

    giro_alternado_tornillo(LIMPIEZA_TIEMPO_SECADO_FINAL_s , LIMPIEZA_RPM_HUSILLO);
//	dejo en situacion aire los conductos
   	operar_ValvLimpieza (LIMPIEZA, Aire);	 //LIMPIEZA a Aire

	parar_Compresor();

	SERIAL_ECHOLN(MSG_LIMPIEZA_FINALIZADA);
    SERIAL_ECHOPAIR(MSG_TIEMPO_LIMPIEZA,(float)(millis()-t_inicio)/1000);
	SERIAL_ECHOLNPGM(_SEG);
	sonidos(9);	  // AVISO de finalizacion.
	lcd_setstatusPGM(PSTR(MSG_LIMPIEZA_FINALIZADA),-1);
}


//************************************************* giro_alternado_tornillo *********************
// Movimiento del tornillo del extrusor, gira alternativamente en los dos sentidos
// en intervalos calculados segun la ratio  LIMPIEZA_RATIO_TIEMPO_HUSILLO_RETRO,
// pero siempre que se de LIMPIEZA_MIN_VUELTAS_RETRO vueltas en retroceso como minimo.
// retardo_ms = 6/1000 * tiempo_seg : Es un delay de tiempo para el cambio de sentido (6 por mil)
  // tiempo_seg =segundos
  // RPM =Revoluciones por minuto
  
  
//  !!!! REVISAR !!!! la entrada al girar-tornillo() ha cambiado

  void giro_alternado_tornillo(int tiempo_seg, int RPM) {
	unsigned int tiempo_ms,tti_ms, ti_ms;		//tiempo sentido inverso (dirmodo=LOW)
	unsigned int tramos;   //numero de tramos
	float retardo_ms = 10 * tiempo_seg ;  //milisegundos pausa para el cambio de sentido

	tiempo_ms = tiempo_seg * 1000L - retardo_ms;   					//tiempo milisegundos total de marcha motor
	tti_ms = (unsigned int)(tiempo_ms /100 * LIMPIEZA_RATIO_TIEMPO_HUSILLO_RETRO);  	//tiempo milisegundos total de inversion

//  SERIAL_ECHOLNPAIR("tti_ms: ", tti_ms);

  tramos = tiempo_ms / tti_ms;    //numero de inversiones

//  SERIAL_ECHOLNPAIR("tramos: ", tramos);

// Afinamos. Al menos debe de dar LIMPIEZA_MIN_VUELTAS_RETRO vueltas en retroceso. Si no disminuyo el numero de tramos.
	for (int x=tramos; x>0; x--){
		ti_ms=tti_ms/tramos;	//milisegundos de inversion por tramo
		if (ti_ms >= LIMPIEZA_MIN_VUELTAS_RETRO*60000L/RPM) 	break;
		if (tramos > 1) tramos-=1;
	}
//	  SERIAL_ECHOLNPAIR("tramos: ", tramos);
	tiempo_ms = tiempo_ms / tramos;   //milisegundos total del tramo

//  SERIAL_ECHOLNPAIR("tiempo_ms: ", tiempo_ms);

	ti_ms = tti_ms / tramos;		//milisegundo en inversion por tramo
	for (int x=0;x < tramos;x++){

//   SERIAL_ECHOLNPAIR("+motor ms: ",tiempo_ms - ti_ms);

		 // Gira en avance un tiempo (tiempo_ms - ti_ms)
		 
		girar_tornillo(tiempo_ms - ti_ms, RPM, 1);
		 //espera a detenerse el motor
		delay(retardo_ms / tramos);
		 // Gira en retroceso un tiempo (ti_ms)
//  SERIAL_ECHOLNPAIR("-motor ms: ",ti_ms);

		girar_tornillo(ti_ms*, RPM, -1);
	}
}

#endif  // fin  PASTAPRINT_LIMPIEZA
...
void sync_plan_position_e() {
  planner.set_e_position_mm(current_position[E_CART]);
}
...
static int x_home_dir(const int extruder) {
  return extruder ? X2_HOME_DIR : X_HOME_DIR;
}
...
void do_blocking_move_to(const float rx, const float ry, const float rz, const float &fr_mm_s/*=0.0*/) {
  ...
  // Si Z necesita subir, lo hacemos antes de mover XY
  ...
  // Si Z necesita bajar, lo hacemos despues de mover XY
  if (current_position[Z_AXIS] > rz) {
    feedrate_mm_s = z_feedrate;
    current_position[Z_AXIS] = rz;
    buffer_line_to_current_position();
...
static void print_bilinear_leveling_grid() {
  ...
  [](const uint8_t ix, const uint8_t iy) {
    return z_values[ix][iy];
  }
                );
}
...
//....................................Aqui podria modificarse la velocidad
...
#if ENABLED(PASTAPRINT)
/**
	Calcula el tiempo (milisegundos) previsto para hacer el movimiento 
	desde current_position (x,y) a  destination (x,y)
*/
float  gcode_time_next_move() {
	float vX,vY,hipo;
	    if (parser.seen(axis_codes[X_AXIS]))	vX = parser.value_axis_units(X_AXIS);
		if (parser.seen(axis_codes[Y_AXIS]))	 vY = parser.value_axis_units(Y_AXIS);
		
		if (axis_relative_modes[0] || relative_mode){  
				
//			SERIAL_PROTOCOLPAIR_F(" X=", vX);
//			SERIAL_PROTOCOLPAIR_F(" Y=", vY);

			hipo= HYPOT(vX,vY);

	  }else{
//		SERIAL_PROTOCOLPAIR_F(" X1-X0=", vX);
//		SERIAL_PROTOCOLPAIR_F("-", current_position[X_AXIS]);
//		SERIAL_PROTOCOLPAIR_F("=", vX -current_position[X_AXIS]);
//		SERIAL_PROTOCOLPAIR_F(" Y1-Y0=", vY);		  
//		SERIAL_PROTOCOLPAIR_F("-", current_position[1]);
//		SERIAL_PROTOCOLPAIR_F("=", vY-current_position[1]);		  
		  
		hipo= HYPOT(vX-current_position[X_AXIS],vY-current_position[1]);
    }

//	SERIAL_PROTOCOLPAIR_F(" dist=", hipo );
//	SERIAL_PROTOCOLPAIR_F("\tmm/s=", feedrate_mm_s );
	
	return  hipo * 1000 /feedrate_mm_s;
...
// La variable G1_E_modo contiene el modo de acción de la linea anterior procesada
//Z_actual= parser.value_axis_units(Z_AXIS)

if (parser.seen('E')) {
	// primero veo si se trata de una retracción o extrusion
	if ( parser.value_axis_units(E_AXIS) < 0 ||
					(!axis_relative_modes[E_CART] && E_anterior_mm3 > parser.value_axis_units(E_AXIS))) {
    ...
    apagar_UV();  //sin luces
		...
		// --------------------------------para informe por monitor
		if (debug_SI){
			SERIAL_ECHO('<');
			SERIAL_ECHO(command_queue[cmd_queue_index_r]);  //listo en pantalla las lineas G0_G1
			SERIAL_PROTOCOLLN(" ");
		}	// --------------------------------fin informe por monitor

		// preguntamos cual era el modo anterior para ver de donde venimos
		if  ( G1_E_modo != RETRACCION){
			//es la primera linea en retraccion
				planner.synchronize();
				// actualizo modo .
				G1_E_modo = RETRACCION;
		}		
				
	}else {
	//************ CASO DE EXTRUSION EN AVANCE
	//	recuperar();
	
#if ENABLED(PRESPRINT)	
   if (!HAY_PRESION) activar_Compresor();
#endif
		
#if ENABLED(UVPRINT)
		if (parser.codenum != 0)encender_UV();  // Si se trata de gcode_G0 no hay luz UV 
#endif
		// --------------------------------para informe por monitor
		if (debug_SI){
			SERIAL_ECHO('>');   //confirmo que esta tratada en este modo
			SERIAL_ECHO(command_queue[cmd_queue_index_r]);  //listo en pantalla las lineas G0_G1
			SERIAL_PROTOCOLLN(" ");
		}	// --------------------------------fin informe por monitor
		
		// Primero veo si es la primera linea en EXTRUSION preguntando por el E_modo actual
		if  ( G1_E_modo != EXTRUSION){
				planner.synchronize();
				// actualizo modo .
				G1_E_modo = EXTRUSION;
		}
		...
		// si no es relative mode guardo el valor de E
		if (!axis_relative_modes[E_CART]) E_anterior_mm3 = parser.value_axis_units(E_AXIS);
		
}else {
	//************ CASO DE SOLO MOVIMIENTO (NO EXTRUSION	Y NO RETRACCION)
		#if ENABLED(UVPRINT)
			//Calculo el tiempo necesario para el viaje para apagar o no UV
			if (gcode_time_next_move() > UV_LAPSO_MINIMO_MILLIS){
				//		SERIAL_PROTOCOLPAIR("time move=", gcode_time_next_move() );
				apagar_UV();
			}	
		#endif	
		...
		// --------------------------------para informe por monitor
		if (debug_SI){
			SERIAL_ECHO('_');	//confirmo que esta tratada en este modo
			SERIAL_ECHO(command_queue[cmd_queue_index_r]);  //listo en pantalla las lineas G0_G1
			SERIAL_PROTOCOLLN(" ");
		}	// --------------------------------fin informe por monitor
		
		//  veo si es la primera linea preguntando por el E_modo actual
		if (G1_E_modo != MOVIMIENTO){
			planner.synchronize();
			// actualizo modo .
			G1_E_modo = MOVIMIENTO;	
		//	if (gcode_time_next_move() > RETRACT_MINIMO_MILLIS)  cierra_obturador();				
		}

	}
		if (!debug_SI)	SERIAL_ECHOLN(command_queue[cmd_queue_index_r]);    //si no hay debug muestro la linea completa
...
#define GO_SRC current_position
    ...
    tmp_fr_mm_s = parser.linearval(
	) > 0.0 ? MMM_TO_MMS(parser.value_feedrate()) : feedrate_mm_s;
...
#if ENABLED(PRESPRINT)
// !! en este caso debe desactivarse FWRETRACT !!
/**
   G10 - Pone en marcha el Compresor.  Solicita presion a la tarjeta UNO
*/
inline void gcode_G10() {
	SERIAL_ECHOLN("G10 PRESPRINT");
	
//	prueba_cierra_obturador();	
	cierra_obturador();
	activar_Compresor();
	}
...
inline void gcode_G12() {		//movimiento para la limpieza de Boquilla
  // Don't allow nozzle cleaning without homing first
  
  	cierra_obturador();   //para eliminar presion en la boquilla
	
#if ENABLED(OBTU_TORN)
		half_open_obturador(obturador_pwm_max, -1); //paso a OPEN
 #endif
	...
	if (debug2_SI) SERIAL_PROTOCOLLN("Limpia " MSG_NOZZLE);
	if ( axis_unhomed_error(parser.seen('X'))){ homeaxis(X_AXIS);}  //inicia en X home
...
#if  ENABLED(PASTAPRINT_LIMPIEZA)
/**		Para PASTAPRINT  creamos un gcode nuevo
   G13: limpieza de conductos y extrusor
*/
inline void gcode_G13() {
  limpieza();
  }
...
/**
   ...
   G19: Select Plane YZ
*/
inline void gcode_G17() {
  set_workspace_plane(PLANE_XY);
}
inline void gcode_G18() {
  set_workspace_plane(PLANE_ZX);
}
inline void gcode_G19() {
...
FORCE_INLINE void gcode_G54() {
  gcode_G54_59();
}
FORCE_INLINE void gcode_G55() {
  gcode_G54_59();
}
FORCE_INLINE void gcode_G56() {
  gcode_G54_59();
}
FORCE_INLINE void gcode_G57() {
  gcode_G54_59();
}
FORCE_INLINE void gcode_G58() {
  gcode_G54_59();
}
FORCE_INLINE void gcode_G59() {
  gcode_G54_59(parser.subcode);
}
...
//  if (all_axes_known()) { // sustituyo esta por la anterior para hacer solo homing si se desconoce la posicion
#if ENABLED(DEBUG_LEVELING_FEATURE)
    if (DEBUGGING(LEVELING)) {
...
void echo_not_entered() {
  SERIAL_PROTOCOLLNPGM(" not entered.");
}
...
/**
   G90 y G91
			G90 pone el modo de referencias absolutas
			G91 pone el modo de referencias relativas
   La razon de ponerlas aqui es solo para poder indicar por monitor el modo en que se trabaja
*/
inline void gcode_G90() {
	relative_mode = false;
	planner.synchronize();
	SERIAL_ECHOLNPGM("Modo absoluto");
}

inline void gcode_G91() {
	relative_mode = true;
	planner.synchronize();
	SERIAL_ECHOLNPGM("Modo relativo");
}
...
void gcode_M8(){
// secuencia de melodia
unsigned char	sec=0;
	if (parser.seen('S'))  sec = parser.byteval('S',0); 
sonidos(sec);
}

 /*
Situa el carro extrusor en el centro de la cama
*/
void gcode_M9(){
if ( axis_unhomed_error(parser.seen('X')|| parser.seen('Y'))) return;
feedrate_mm_s = 50;	
destination[0]=X_BED_SIZE /2;
destination[1]=Y_BED_SIZE /2;
prepare_move_to_destination();       //Realiza el movimiento

}

 void gcode_M10(){ inicia_PASTAPRINT(); }  
 
/**  31/5/24
	NUEVO G_CODE  de MULTIMAT_PRINTER (No existe en Marlin)
	M11:  Bajar la cama
	Este comando se puede ejecutar mediante comando o con el pulsador incado para bajar cama.
	- No se puede bajar la cama si se esta imprimiendo.
	- Primero baja 20mm y despues de 20 en 20 mm mientras se mantenga pulsado.
	- Dispone de tope fin de carrera (comprobar si funciona)
*/
void gcode_M11(){
	// No se puede si estoy imprimiendo.
//	if (IsRunning()) return;  
	if (card.sdprinting) return;  
	
	sonidos(3);	// sonido de movimiento de cabezal
	endstops.enable(true); // Enable endstops for next homing move
	float Z_parte = current_position[Z_AXIS] +20;  //baja primero 20mm despues de 5 en 5 mm

//mira si no se esta imprimiendo y se continua pulsando pulsador de bajar cama
	if (!card.sdprinting ){
	    while(READ(Z_MAX_PIN)) {
			do_blocking_move_to_z(Z_parte, NOZZLE_PARK_Z_FEEDRATE);
			planner.synchronize();
			Z_parte+= 20;
			if ( READ(BAJAR_CAMA_PIN) != LOW) break;
	    }
	}
	planner.synchronize();
    gcode_M18_M84(); //disable stepper Z
}
	...
	/**
		NUEVO G_CODE  de MULTIMAT_PRINTER (No existe en Marlin)
		M12:  PRUEBAS DE SERVOS
	*/
	void gcode_M12(){

	sonidos(0);	// sonido de movimiento de cabezal
	bool res=false;
	do{
		do{
			res= stop_mensaje("SERVOS>>aire",2);
			if (!res) {  //pulsar res=false
				operar_ValvLimpieza(DEPOSITO, Aire);
				operar_ValvLimpieza(LIMPIEZA, Aire);
				break;
			}
			res= stop_mensaje("SERVOS>>bloq",2);
			if (!res) {  //pulsar res=false
				operar_ValvLimpieza(DEPOSITO, Bloq);
				operar_ValvLimpieza(LIMPIEZA, Bloq);
				break;
			}
			res= stop_mensaje("SERVOS>>agua",2);
			if (!res) {  //pulsar res=false
				operar_ValvLimpieza(DEPOSITO, Agua);
				operar_ValvLimpieza(LIMPIEZA, Agua);
				break;
			}

		}while (1);
	} while (stop_mensaje("¿Fin?",2));
...
#if ENABLED(PASTAPRINT) && ENABLED(CTCHO_EMBOLO)

/*		
	****************  sangrado_manual_extrusor(float pasta_mm3)  ***********************
	Gira el tornillo del cartucho a la velocidad de la variable global <sangrado_mm3_min> 
	 (puede modificarse con M13) sangrando una cantidad de pasta <pasta_mm3>
	- Se acciona pulsando el pulsador manual de sangrado SANGRADO_MANUAL_PIN (en LOW para marcha)
	- El sentido solo está determinado por el pin SENTIDO_AVANCE_MANUAL_PIN, que se maneja con el conmutador manual.
		Si esta pulsado es LOW = extrusion, si HIGHT retracción  
	- Se repite mientras esté pulsado el pulsador de marcha (pin motor) 
	- (Añadido 31/3/23) y modificado para MULTIMAT para actuar sobre el obturador 
*/
void sangrado_manual_extrusor(float pasta_mm3){
	int dir;
	if (pasta_mm3 < 0) pasta_mm3 =-pasta_mm3; //solo admite pasta positiva
		
	lcd_reset_alert_level();  //pongo el nivel a cero
	#if ENABLED(OBTU_TORN)
		half_open_obturador(obturador_pwm_max, 1);  //Accion 1 paso a Half
	#endif
	//Veo en que direccion hay que mover
	if (READ(SENTIDO_AVANCE_MANUAL_PIN) == LOW) {  //Extrusion
		lcd_setstatusPGM(PSTR(MSG_EXTRUSIONAR_TORNILLO) , -1);
	}else{								// Retraccion
		pasta_mm3 =-pasta_mm3;
		lcd_setstatusPGM(PSTR(MSG_RETRAER_TORNILLO ), -1);
	}
	 
	 // current_position[E_AXIS]=0;
	
	SERIAL_ECHOPAIR(" Sangrado ", pasta_mm3);
	SERIAL_ECHOPAIR(" mm3 a ",(int) sangrado_mm3_min);
	SERIAL_ECHOLNPGM(" mm3/min ");
	//--------------------------------------------	
			feedrate_mm_s = MMM_TO_MMS(sangrado_mm3_min);
			set_destination_from_current();
			destination[E_AXIS] += pasta_mm3;  
...
#endif		//(PASTAPRINT) && (CTCHO_EMBOLO)

#if ENABLED(PASTAPRINT)
/** 31/5/24
	NUEVO G_CODE  gcode_M13 de PASTAPRINT para el sangrado del extrusor (M13 No existe en Marlin).
	Es igual que el sangrado manual pero en forma de gcode con parametros que permite 
	extruir o retraer cualquier cantidad de pasta. (Con limites en version LIQUID)
		Parametros:
		"S" velocidad de extrusion en mm3/min. 
			-Por defecto S = sangrado_mm3_min. 
			-El valor entrado (positivo) modifica la variable global <sangrado_mm3_min> para la sesion actual
		"M" cantidad de pasta en mm3. 
			-Por defecto 100 mm3 o la ultima entrada
			-Si el valor entrado es negativo mueve el embolo en retroceso (retracción)
			-Si se ha llamado a M13() con el pulsador manual SANGRADO_MANUAL_OK_PIN, 
				la direccion se obtiene del valor del interrutor SENTIDO_AVANCE_MANUAL_PIN
		"E" -Seleccion del motor E=0 Solo CARTUCHO   E=1 Solo BOQUILLA  E=sin entrada o 0tro valor  Funcionan ambos	
*/

 void gcode_M13() {
     float mm3 = 100;
	 int dir=1;
	//Si sangrado manual esta activado uso la posicion del interruptor de sentido
	//para darle signo de direccion a a operacion
	if SANGRADO_MANUAL_ACTIVADO {
		(READ(SENTIDO_AVANCE_MANUAL_PIN) == LOW) ? dir=1 : dir=-1; // LOW= EXTRUSION HIGH=RETRACCION
		mm3= ABS(mm3) * dir;   //asigno la posición del interruptor   
	}	

	planner.synchronize();	
	int motor=0, mm3_min = sangrado_mm3_min;

	if (parser.seen('S')){ 
		mm3_min = parser.intval('S',sangrado_mm3_min);
		if (mm3_min < 10) return;
		sangrado_mm3_min=  mm3_min;
	}
#if ENABLED(EXTRUSOR_MIX)
	if (parser.seen('E')){ motor = parser.intval('E',2); }
#endif	

	if (parser.seen('M')) mm3 = parser.floatval('M',mm3); 
	
#if ENABLED(PRESPRINT)
//en el caso de presion neumatica mm3 significa milisegundos de apertura del obturador siempre
//en sentido de extrusion. No hay retracciones.
	if (!HAY_PRESION)  activar_Compresor();
	...
	#if ENABLED(OBTU_TORN)
		half_open_obturador(obturador_pwm_max, 1);   //Paso a pos normal Half. 
		delay(mm3);
		return;
	#endif
#endif
	
	#if ENABLED(LIQUID) && DISABLED(JERINGA)
		float	ope_upasos_liquido=ABS(mm3)*planner.axis_steps_per_mm[E_AXIS];
		
		if (mm3 >0) {
				//como es extrusion antes veo si hay limite con un margen del 2%
				if (liquid_posicion_upasos < liquid_total_upasos * 0.02) return;
				NOMORE(ope_upasos_liquido,liquid_posicion_upasos-liquid_total_upasos * 0.02);
				mm3=ope_upasos_liquido/planner.axis_steps_per_mm[E_AXIS];
		}else{
				//como es retraccion veo limites con un margen del 2%
				if (liquid_posicion_upasos>liquid_total_upasos * 0.98) return;
				NOMORE(ope_upasos_liquido,liquid_total_upasos * 0.98 - liquid_posicion_upasos);
				mm3=-ope_upasos_liquido/planner.axis_steps_per_mm[E_AXIS];
		}	
	#endif	
	
	if (parser.seen('M') || parser.seen('S') || parser.seen('?') ){
		if (mm3 >0) {
			SERIAL_ECHOPAIR_F(MSG_EXTRUSIONAR_TORNILLO ": ", mm3);
		}else{
			SERIAL_ECHOPAIR_F(MSG_RETRAER_TORNILLO ": ", mm3);	
		}
		SERIAL_ECHOPAIR(" mm3 a ",(int) sangrado_mm3_min);
		SERIAL_ECHOPGM(" mm3/min ");
		if (parser.seen('?')) return;
	}
	
#if ENABLED(EXTRUSOR_MIX)	
	if (motor ==0) {SERIAL_ECHOPGM(" en CARTUCHO");}
	if (motor ==1) {SERIAL_ECHOPGM(" y BOQUILLA");}
#endif
...
/*
ejemplo para crear una linea de gcode_G0_G1
*/
void print_linea(int Gn, float vf,  float d1, float d2, float d3){;
char bufer[35], bufn[9]; ;
	(Gn==1) ? strcpy(bufer,"G1 F") : strcpy(bufer,"G0 F");
    dtostrf(vf, -5,0, bufn);
	strcat(bufer, bufn);  
	strcat(bufer, " X");
	dtostrf(d1, -6,2, bufn);
	...
	dtostrf(d2, -6,2, bufn);
	strcat(bufer, bufn);   
	(Gn==1) ? strcat(bufer, " E") :  strcat(bufer, " Z");
	dtostrf(d3, -6,3, bufn);
	strcat(bufer, bufn);  	
 ...
 bool forma_linea_G1(float mm_min, float xx, float yy, float mm3){
	 
planner.synchronize();	 

	if ( debug2_SI)  print_linea(1, mm_min, xx,yy,mm3) ;
	
  feedrate_mm_s = MMM_TO_MMS(mm_min);	
  destination[0]=xx;
  destination[1]=yy;
  destination[3]+=mm3;
  prepare_move_to_destination();       //Realiza el movimiento
...
void forma_linea_G0(float mm_min, float xx, float yy, float zz){

planner.synchronize();	

	if ( debug2_SI)  print_linea(0, mm_min, xx,yy,zz) ;
...
/**
	NUEVO G_CODE  de PASTAPRINT (M50 No existe en Marlin)
	M50: CALIBRACION VOLUMETRICA DEL EXTRUSOR.
	Ensayos de calibracion para obtener el "Rendimiento de extrusion" comparando la cantidad
	teorica con la realmente extruida. 
	El ensayo solo es válido para el embolo y boquilla montada en la máquina. (NO actualiza ningun dato en la EEPROM M500)
	
	Parametros:
		M<int>		Cantidad a extruir (default = 500 mm3 )
		N<int>  	Numero de tramos (default = 1)
		V<int>  	Velocidad de extrusion en mm por segundo de hilo extrusionado. (default= VLINEAL_MAX_EXTRUSION_MM_MIN / 2 /60)
					!atencion! Dividiendo por la seccion del hilo se obtiene la velocidad para el gcode G1
		S <int>	    Velocidad de extrusion en mm3 por minuto. 	Por defecto S=sangrado_mm3_min 
		I<int>  	Intervalo de velocidades de extrusion. (default= VLINEAL_MAX_EXTRUSION_MM_MIN / 8)
		D<float>	Densidad del material gr/cm3. (default= 1gr/cm3)

	Metodo del ensayo:
		Previamente se habra obtenido el peso de la cantidad M mm3 y su densidad.
		Realiza la extrusion (con rendimiento de extrusion = 100), una por tramo de velocidad, de M mm3 a S mm3/min con la boquilla y embolo actuales 
		a escape libre, con objeto de medir el peso depositado en el recipiente de tara conocida.
		Se obtienen los mm3 realmente obtenidos para cada tramo de velocidad, y calculamos el rendimiento 
		de la extrusion con la formula:  
						factor_rendimiento_extrusion  =  real(gr) / teorico(gr) = real(mm3) / teorico(mm3)
		Repetir el ensayo habiendo modificado la variable de rendimiento de extrusión comprobando que lo real coincide 
		sensiblemente con lo medido. Hay que tener en cuenta que se hace a boquilla libre, sin obstaculos.				
	Variables:
		punto inicial en X=0 Y=cualquiera   
		Distancia entre recipientes en el eje X = 40 mm.
	 !!Atencion!! Previamente hay que haber dejado los recipiente debajo!!
/*/

void gcode_M50() {

	SERIAL_ECHOLNPGM("\nM50 CALIBRACION VOLUMETRICA DEL EXTRUSOR ");
//	SERIAL_ECHOLNPGM(MSG_M50_CALIBRACION);	
	
int  	mm3 = 200,
		n_tramos = 1;
float 	mm3_min = sangrado_mm3_min,
		intervalo_mm3_min = mm3_min / 6,
		d_boquilla = planner.diam_boquilla,
		densidad = 1;
		
int8_t  distanciaX= 40;
float 	S_boquilla, S_embolo, peso_teorico, posicionX=0;
int 	pos=0;									  

//    M50 M>mm3> S<mm3/min> N<num.tramos> I<tramo mm3/min> D<densidad>1 
	if (parser.seen('M'))  mm3 = parser.intval('M',mm3); 
	if (parser.seen('S'))  mm3_min = parser.floatval('S',mm3_min); 
	if (parser.seen('N'))  n_tramos = parser.intval('N',n_tramos); 
	if (parser.seen('I'))  intervalo_mm3_min = parser.floatval('I',intervalo_mm3_min); 
	if (parser.seen('D'))  densidad= parser.floatval('D',densidad); 
	
	if (parser.seen('?') || debug2_SI==true ) {
		SERIAL_ECHOPAIR(" M50 M<mm3=",mm3);
		SERIAL_ECHOPAIR("> S<mm3/min=",mm3_min);
		SERIAL_ECHOPAIR("> N<num.tramos=",n_tramos);
		SERIAL_ECHOPAIR("> I<+tramo mm3/min=",intervalo_mm3_min);
		SERIAL_ECHOPAIR_F("> D<densidad=",densidad);
		SERIAL_ECHOLN(">");
		if (parser.seen('?')) return;
	}
 	

	S_boquilla =  CIRCLE_AREA(d_boquilla * 0.5);
	S_embolo =  CIRCLE_AREA(planner.filament_size[0] * 0.5);
  
  	BUZZ(1000,20);    //aviso movimiento


  	#if ENABLED(UVPRINT)
		apagar_UV();  //por si acaso apago luces
	#endif

/*    planner.synchronize();
// primero se ha de haber hecho homing del eje X
	if ( axis_unhomed_error(parser.seen('X'))){ }
	planner.synchronize();
*/
    homeaxis(X_AXIS);
	planner.synchronize();
	posicionX = current_position[X_AXIS];	
	
	if (densidad > 0)  peso_teorico = 100 /1000 * densidad ;
 
	SERIAL_ECHOPAIR_F("D.Embolo:", planner.filament_size[0]);
	SERIAL_ECHOPAIR_F("  " MSG_NOZZLE ":", d_boquilla);
	SERIAL_ECHOPAIR_F(" (S=", CIRCLE_AREA(d_boquilla * 0.5));
	SERIAL_ECHOPAIR( "mm2)  Extrusion:",mm3);
	SERIAL_ECHOPAIR_F(" mm3  Densidad:", densidad);		
	SERIAL_ECHOPAIR_F("gr/cm3  Peso:", peso_teorico);	
	SERIAL_ECHOLN("gr");
	SERIAL_ECHOPGM("Tramo");
	SERIAL_ECHOPGM("  mm3/min");
	SERIAL_ECHOPGM("  mm/min");
	SERIAL_ECHOPGM("  mm/seg");
	SERIAL_ECHOPGM("    seg   ");	
	SERIAL_ECHOLNPGM("   peso gr");	
	
	pos=0;	
	millis_t time_in;
	
	for (int e=1;e < n_tramos+1;e++){ 	
		BUZZ(200,500);
		delay(4000);
		
		SERIAL_ECHOPAIR("  ",e);
		SERIAL_ECHO("\t");
		SERIAL_ECHO_F(mm3_min,0);			
		SERIAL_ECHO("\t");
		SERIAL_ECHO_F(mm3_min/S_boquilla,0);
		SERIAL_ECHO("\t");
		SERIAL_ECHO_F(mm3_min/S_boquilla/60,1);


		SERIAL_ECHO("\t ");	
		
		do_blocking_move_to_x(posicionX, 60);  //Mueve boquilla a 60 mm/s
		planner.synchronize();
//		extruir_tornillo( 2,100, mm3_m, 1);
		
// en sustitucion de extruir_tornillo

		time_in = millis();
		current_position[X_AXIS] = posicionX;	
		forma_linea_G1( mm3_min/S_embolo,  current_position[X_AXIS], current_position[Y_AXIS], mm3 );
		
		mm3_min += intervalo_mm3_min;
		posicionX += distanciaX;
		
		planner.synchronize();
		
		SERIAL_ECHO_F((float)((millis()-time_in)* 0.001),3); 
		SERIAL_ECHOLN("");		
...
// Nuevo gcode M51  ejemplo:( G1 F1800 X88.673 Y88.014 E0.11261 )     ]


/**
	NUEVO G_CODE  de PASTAPRINT (M51 No existe en Marlin)
	M51: CALIBRACION DE LINEAS .
	Como se trata de un gcode que se activa controlado por el sistema de anillo de Marlin
	no se puede usar la inyeccion de nuevos gcodes desde esta rutina, por tanto
	se llaman a las funciones directamente en tiempo real de ejecución.
	Ensayos de calibracion para determinar el rendimiento de la extrusion y el comportamiento 
	de la retracción y acabado en esquinas. 
	Realiza un numero N de rectangulos concentricos

	Parametros:
		- V <float>  Velocidad lineal para linea con extrusion (E!=0) en mm/seg
		- N <int>	 Numero de figuras concentricas. 			Por defecto = 2 
		- H <float>	 Altura de capacity							Por defecto H=0.2 mm	
		- X <float>	 Posicion inicial en el eje X   			Por defecto X0=50
		- y <float>	 Posicion inicial en el eje Y   			Por defecto Y0=50		-
		- A <float>	 Lado eje X del rectangulo					Por defecto A=100
		- B <float>	 Lado eje Y del rectangulo					Por defecto B=100	
		- I <float>	 Decalaje entre rectangulos en eje X 		Por defecto a=5
		- J <float>	 Decalaje entre rectangulos en eje Y 		Por defecto b=5		
		- ?          Lista parametros actuales
*/

void gcode_M51() {
int  	n_capas = 1, n_bloques = 2;
float 	mm_seg = VLINEAL_MAX_EXTRUSION_MM_MIN / 2 /60,
		d_boquilla = planner.diam_boquilla,
		capa = 1,
		X0 = 20,
		Y0 = 20,
		lado_x = 120,
		lado_y = 120,
		paso_x = -10,  //En negativo hace los siguientes cuadros por fuera 
		paso_y = -10;

float  mm2, x0, y0, x1, y1, zz; 										   

//     M51 V<mm/s> C<num.capas> N<num.cuadros> V<mm/min> H<capa> x<X0> y<Y0> A<LadoX> B<LadoY> a<PasoX> b<PasoY>;

	if (parser.seen('V'))  mm_seg= parser.intval('V',mm_seg); 
	if (parser.seen('C'))  n_capas= parser.intval('C',n_capas); 
	if (parser.seen('N'))  n_bloques= parser.intval('N',n_bloques); 
	if (parser.seen('H'))  capa= parser.floatval('H',capa); 		
	if (parser.seen('X'))  X0= parser.floatval('X',X0); 
	if (parser.seen('Y'))  Y0= parser.floatval('Y',Y0); 	
	if (parser.seen('A'))  lado_x= parser.floatval('A',lado_x); 
	if (parser.seen('B'))  lado_y= parser.floatval('B',lado_y); 
	if (parser.seen('I'))  paso_x= parser.floatval('I',paso_x); 
	if (parser.seen('J'))  paso_y= parser.floatval('J',paso_y); 
	
	if (    true    || parser.seen('?') || debug2_SI==true ) {
		SERIAL_ECHOPAIR_F(" M51 V<mm/seg=",mm_seg);
		SERIAL_ECHOPAIR("> C<num.capas=",n_capas);
		SERIAL_ECHOPAIR("> N<num.cuadros=",n_bloques);
		SERIAL_ECHOPAIR_F("> H<capa=",capa);
		SERIAL_ECHOLN(">");
		SERIAL_ECHOPAIR_F("\t X<X0=",X0);
		SERIAL_ECHOPAIR_F("> Y<Y0=",Y0);
		SERIAL_ECHOPAIR_F("> A<LadoX=",lado_x);
		SERIAL_ECHOPAIR_F("> B<LadoY=",lado_y);
		SERIAL_ECHOPAIR_F("> I<PasoX=",paso_x);
		SERIAL_ECHOPAIR_F("> J<PasoY=",paso_y);
		SERIAL_ECHOLN(">");
		if (parser.seen('?')) return;
	}
 
mm2 = planner.diam_boquilla * capa;

planner.synchronize();

//pongo a nivel 2 el debuger
// marlin_debug_flags = 2;
// debug2_SI=true;
#if ENABLED(OBTU_SOLE)
		cierra_obturador();	
		 BUZZ(200,1000);    //aviso !!QUITAR EL GATILLO!!
		sonidos(4);
		 BUZZ(200,1000);
	#endif

//_______Inicia en home
gcode_G28(true);
//G90  ; referencias absolutas
gcode_G90();
		...
		if HAY_PRESION SERIAL_ECHOLN(" Hay presión");	
			...
			activar_Compresor(); 
		SERIAL_ECHO(" Compresor activado ");	 
		for (int i;i<10;i++){
			delay(2000);
			SERIAL_ECHO(".");
			if HAY_PRESION break;
		}
			if HAY_PRESION SERIAL_ECHOLN(" Hay presión");		
			else {
				parar_Compresor();
				abre_obturador();
				SERIAL_ECHOLN(" No hay presion");
				return;
			}
		}
	#endif
 ...
 // levanto la altura de capa a 500 mm3/min
forma_linea_G0(500,0,0,capa);

 for (int c = 1; c < n_capas +1 ; c++){

		for (int i=0; i< n_bloques; i++){
			//linea 0    posiciono origen inicio cuadro
		//	fwretract.retract(true);
			x1=X0+paso_x*i;
			y1=Y0+paso_y*i;
			forma_linea_G0(3600,  x1,  y1, capa*c);
			
			//linea 1   horizontal
		//	fwretract.retract(false);
			x0=X0+lado_x-paso_x*i;
			y0=y1;	
			forma_linea_G1(mm_seg*60,  x0,  y0, mm2*ABS(x1-x0+y1-y0));

			//linea 2	vertical
			x1=x0;
			y1=Y0+lado_y-paso_y*i;	
			forma_linea_G1( mm_seg*60,  x1,  y1, mm2*ABS(x1-x0+y1-y0)); 
 ...
 forma_linea_G0(1000,10,10, 20);
 #if ENABLED(UVPRINT)
	apagar_UV();
 #endif
 gcode_M18_M84(); //disable stepper Z	 //disable steppers
 abre_obturador();
 parar_Compresor();
}
...
#if ENABLED(PASTAPRINT)
/**
	NUEVO G_CODE  de PASTAPRINT (M16 No existe en Marlin)
	M16 : 	Actualiza parametros importantes para la sesion. 
			Pueden guardarse con la opcion G para otras sesiones
	Set parametros diametro boquilla y %Rendimiento de extrusion.
			Actualiza temporalmente el rendimiento del extrusor y el nuevo 
			Pasos_mm3_hilo real  en la variable Marlin <axis_steps_per_mm[E_AXIS]>				
			Se graba con la opcion "G"
	Parametros:

		"D" Diametro de la boquilla (Toma por defecto = Planner::filament_size[0]

		"R" % Rendimiento del extrusor. Toma por defecto = planner.factor_rendimiento_extrusion 

		'G' Regraba en EEPRON.  M500

		Pasos_mm3_REAL = (PASOS_MM3_TEOR / BQLLA_RENDIMIENTO_EXTRUSION  * 0.01f)

		* SECCION(Diametro_boquilla)
		Si se cambia el %factor_rendimiento_extrusion se actualiza con <actualiza_steps_mm3_E()>
*/

void gcode_M16() {

  // lee los parametros si los hay
  float d_boq, factor_rendimiento_extrusion;
	
 if (parser.seen('D') && parser.seen('R') ) SERIAL_ECHOLNPGM("gcode M16 D<diam.boquilla> R<Porc.Rendimiento extrusor> G<Actualiza EEPROM");
  
	d_boq = (float)parser.floatval('D',0.0f);
  if (d_boq >0 && d_boq<5) {
	  Planner::filament_size[0]=d_boq;
  }else{
	  d_boq = Planner::filament_size[0];
  }

if (parser.seen('R')){ 
		factor_rendimiento_extrusion = (float)parser.floatval('R',planner.factor_rendimiento_extrusion );
	   if (factor_rendimiento_extrusion >0 && factor_rendimiento_extrusion<101) {
			planner.factor_rendimiento_extrusion =factor_rendimiento_extrusion;
			planner.actualiza_steps_mm3_E();		   
	  }else{
		  factor_rendimiento_extrusion = planner.factor_rendimiento_extrusion ;
	  }
	}

// por default cuando se llama a esta rutina se imprime el comando entrado en el monitor
	SERIAL_ECHOPAIR_F("M16  D=", d_boq);	
	SERIAL_ECHOPAIR_F("  R=", factor_rendimiento_extrusion);	
	SERIAL_ECHOLNPAIR_F("% >>" MSG_PASOS_MM3 "=", planner.axis_steps_per_mm[E_AXIS+1]);
	
	//graba en la EEPROM los datos actuales SOLO si se uso la opcion "G" 
	// !PELIGRO! Graba toda la configuracion actual completa = M500
	if (parser.seen('G')){ 
		(void)settings.save();
		SERIAL_ECHOPGM(MSG_CALIBRACION "  >>>EEPROM \t(");
	}
/*
	SERIAL_ECHOPAIR_F(MSG_PASOS_MM3 "(teórico)=", BQLLA_PASOS_MM3_TEOR);
	SERIAL_ECHOLN(")");
	SERIAL_ECHOPAIR_F(MSG_FILAMENT_DIAM "=", d_boq);
	SERIAL_ECHOPGM("mm\t");
	SERIAL_ECHOPAIR_F(MSG_BQLLA_RENDIMIENTO_EXTRUSION  "=", planner.factor_rendimiento_extrusion );
	SERIAL_ECHOLNPAIR_F(" >>>\t" MSG_PASOS_MM3 " (real)=", planner.axis_steps_per_mm[E_AXIS+1] );
*/
}
...
#if ENABLED(PASTAPRINT)
    	lcd_setstatusPGM(PSTR(MSG_HEATING), -1);
#endif
...
inline void gcode_M82() {
  axis_relative_modes[E_CART] = false;
   	SERIAL_ECHOLNPGM("E en Modo absoluto"); 
}
...
inline void gcode_M83() {
  axis_relative_modes[E_CART] = true;
  	SERIAL_ECHOLNPGM("E en Modo relativo");
}
...
/**
   M92: Set axis steps-per-unit for one or more axes, X, Y, Z, and E.
		...
		nota de PASTAPRINT: Para el caso del extrusor <axis_steps_per_mm[3]> debe entrar pasos/mm3 
		pero ya afectado por el % de rendimiento del husillo.
*/
...
inline void report_xyz(const float pos[]) {
  report_xyze(pos, 3);
}
...
/* ************************* half_open_obturador( int valor_pwm, int accion) **********************************
 Se ha observado que el sistema de leva con roldana empleado produce un aumento del volumen disponile 
 en el interior y en consecuencia produce un efecto de succion (retracción) en la boquilla.
 Naturalmente para eso ir a esa posicion debe estar el conducto mas cerrado pero permitiendo el paso de
 la pasta hacia la boquilla normalmente que podriamos denominar "abierta a medias" que denominaremos "half".
 
 Por todo ello normalmenta la posicion del obturador debe ser la de HALF.
 
	************************** 	VERSION MOTOR CC ****************************************
	 Entradas:
     int8_t valor_pwm = numero entre 0 y 255 que indica el nivel de voltaje aplicado al motor. Maximo con 255
	 int  accion: (accion=1)>>>  HALF. Giro horario (recogida de la sirga)ENTREABIERTO posicion normal
				  (accion=-1)>>> ABRIR. Giro antihorario (salida de la sirga ) EXPANSION del conducto 
    	          	 
	 Funcionamiento:
	 El sentido de giro se establece con los pines IN1_PIN y IN2_PIN:
	 		HALF_PIN      High = No esta half     Low = Half
			OPEN_PIN 	  High = No esta Open     Low = Open
		Sentido clockwise    IN1_PIN=LOW IN2_PIN=HIGH  (HALF)    accion=1
		Sentido counterwise  IN1_PIN=HIGH IN2_PIN=LOW  (OPEN)	 accion=-1		
	 Hay dos pines de fin de carrera que controlan la posicion actual:

     El enable del motor (PWM_OBTURADOR_PIN) establece en un pin PWM la velocidad.
	 Si queremos dejar normal el obturador hacemos PWM_OBTURADOR_PIN = LOW o tambien IN1_PIN=IN2_PIN 
*/
void half_open_obturador( int valor_pwm, int accion) {
			
unsigned long t0_millis = millis();	//inicio contador
unsigned long limite = t0_millis + t_max_obturador_ms;	//tiempo maximo absoluto en milisegundos para la operacion.

	  if ((accion>0 & ESTA_OPEN )or ESTA_INDEFINIDO){		//pasa a half
		 digitalWrite(IN1_PIN, LOW);
		 digitalWrite(IN2_PIN, HIGH);

		while (!ESTA_HALF and limite>millis()){
			analogWrite(PWM_OBTURADOR_PIN,valor_pwm); // Send PWM signal to L298N Enable pin
		 }
		 analogWrite(PWM_OBTURADOR_PIN,0); // Disable motor
		 digitalWrite(IN2_PIN, LOW);
		 
		 SERIAL_ECHO_START();
		 SERIAL_ECHOPGM("HALF ");		 
		 SERIAL_ECHOLNPAIR_F("Obt.(ms):",millis()-t0_millis);

	  }else {
   
		  if (accion<=0 & ESTA_HALF) {  		//LO ABRE
			 digitalWrite(IN1_PIN,HIGH );
			 digitalWrite(IN2_PIN,LOW );

			 while (!ESTA_OPEN and limite>millis()){
			analogWrite(PWM_OBTURADOR_PIN,valor_pwm); // Send PWM signal to L298N Enable pin
		 }		 
			 analogWrite(PWM_OBTURADOR_PIN,0); // Disable motor
			 digitalWrite(IN1_PIN, LOW);
...
/* ******************************  posicion_actuar_mordaza(int rpm_tornillo) ****************************
- En el tablero manual el pulsador Montar/Desmontar hace que el pin LIBERAR_MORDAZA_PIN cambie a LOW que al ser 
	detectado por el loop principal hará girar la tuerca partida una vuelta completa hasta que este se encuentre 
	en la posicion de apertura/cierre de la tuerca partida, es decir, hasta que swich MORDAZA_ACCESIBLE es true.
  En ese caso la rutina devuelve true de lo contrario devuelve false.
  Siempre gira en el sentido de retraccion del embolo (para evitar enclavamientos) y a la velocidad del motor indicada.
*/

bool posicion_actuar_mordaza(int rpm_motor){
	
	unsigned long upulse = uPASOS_VUELTA_MOTOR / RELAC_MOTOR_TORNILLO;
	unsigned long stepDelay = 30000000UL / ( rpm_motor * uPASOS_VUELTA_MOTOR);   //microsegundos

//	SERIAL_ECHOPAIR("\t upulse= ", upulse);
//	SERIAL_ECHOLNPAIR(" stepDelay= ", stepDelay);
	WRITE(E0_ENABLE_PIN, LOW);   //enable motor del tornillo
	WRITE(E0_DIR_PIN, HIGH);  //sentido retraccion para liberar mordaza
	int iter = 5;	//cada 5 pasos mira si ha disparado el fin de carrera
	while (!MORDAZA_ACCESIBLE && upulse > iter){
	...
	WRITE(E0_ENABLE_PIN, HIGH);   //disable motor del tornillo
	WRITE(E0_DIR_PIN, LOW); 
	sonidos(4);
	return (MORDAZA_ACCESIBLE ) ? true : false;
}

  /* **************************************** girar_tornillo ****(MODIFICADO 12/05/23)*****************
  Movimiento de giro del tornillo del extrusor del cartucho hasta la posicion de la ventana 
  ENTRADAS:
  float rev_tornillo = vueltas o fraccion de vuelta que dara el tornillo
  float rpm_tornillo = Velocidad de giro del tornillo (Revoluciones por minuto)
  int  direc:  (direc>0) giro antihorario (sentido de empuje del tornillo=LOW)  
 	          (direc=<0) giro horario (clockwise) sentido retraccion  = HIGH
  SALIDA: si llego a la posicion devuelve "true"  y además el swich LIBERAR_MORDAZA_ACTIVADO es true
		y "false" si salio por superacion del tiempo permitido. (Perdio pasos)
  El ENABLE del motor lo activa aqui (E0_ENABLE_PIN=LOW).
  Cada 5 micropasos de motor se lee la situacion 
  del FC_MORDAZA_ACCESIBLE_PIN
*/

bool girar_tornillo( float rev_tornillo, int rpm_tornillo, int direc) {
	  
	bool dir = (direc>0) ? LOW : HIGH;

		WRITE(E0_DIR_PIN, dir);

	unsigned long upulse = (uPASOS_VUELTA_MOTOR  /  RELAC_MOTOR_TORNILLO 	) * rev_tornillo;		//   micropasos a dar
	
	if (upulse < MINIMUM_STEPPER_PULSE) upulse = MINIMUM_STEPPER_PULSE;

//  Este es el tiempo activo=inactivo
	unsigned long stepDelay = (rev_tornillo * 30000000UL / rpm_tornillo  )/ upulse ;   //microsegundos
	if (stepDelay < MINIMUM_STEPPER_DIR_DELAY /1000L) stepDelay = MINIMUM_STEPPER_DIR_DELAY / 1000L;

//	SERIAL_ECHOPAIR_F(" Tornillo>> Rev=", rev_tornillo);  
//	SERIAL_ECHOPAIR(" RPM=", rpm_tornillo);  //DEBUG
	SERIAL_ECHOPAIR_F(" Motor>> rpm=", rpm_tornillo/RELAC_MOTOR_TORNILLO);  
	direc>0 ? SERIAL_ECHOLNPGM(" " MSG_EN_AVANCE): SERIAL_ECHOLNPGM(" " MSG_EN_RETROCESO);
//    SERIAL_ECHOLNPAIR("\tstepDelay(us)= ", stepDelay);		 //DEBUG

    WRITE(E0_ENABLE_PIN, LOW);   //enable motor del tornillo
	delay(50);	//Tiempo para el enable
	
//	millis_t ini_millis =millis();

	int iter = 5;
	while (!MORDAZA_ACCESIBLE && upulse>0){
		for (int i = 0; i < iter; i++) {
			  WRITE(E0_STEP_PIN, HIGH);
			  delayMicroseconds(stepDelay);
			  WRITE(E0_STEP_PIN, LOW);
...
//	SERIAL_ECHOLNPAIR("\tMotor(upulse)=", upulse);  //DEBUG		
	}
				
	 if (!MORDAZA_ACCESIBLE) {
		  SERIAL_ECHOLNPGM("ERROR. E0 perdio pasos");
			sonidos(2);
	  }
//    SERIAL_ECHOLNPAIR("\ttiempo (s)= ", (millis()-ini_millis)/1000);
...
#if  ENABLED(EXTRUSOR_MIX) 
  //**************************************** extruir_tornillo *********************
  /* Movimiento de giro del tornillo del extrusor de CARTUCHO y el de la BOQUILLA de forma sincronizada
	   int8_t motor   	= motor=2  Ambos motores     motor=0  Solo motor CARTUCHO    motor=1 Solo motor BOQUILLA
	   int pasta_mm3 	= Volumen de pasta a imprimir. Solo admite integer
	   float mm3_min 	= Velocidad de extrusion de la pasta mm3/min
	   int  direc		=  (direc>0) giro antihorario (extrusion=LOW)  
							(direc=<0) giro horario (retraccion  = HIGH

   E0 se refiere al motor del tornillo del CARTUCHO
   E1 se refiere al motor del tornillo de la BOQUILLA
   El ENABLE del motor se activa con LOW.
*/
  void extruir_tornillo( int8_t motor, int pasta_mm3, float mm3_min, int direc) {
 int8_t maxima_continuidad_mm3 = 50;  //maxima cantidad para marcha combinada
 millis_t 	 t_millis=millis();	 
 bool dir = (direc>0) ? LOW : HIGH;
 float  paso_B = 1.0, paso_C =1.0;

 unsigned long tramo_pasos, resto;
 /* 
	SERIAL_ECHOPAIR_F(" mm3=", pasta_mm3);  
	SERIAL_ECHOPAIR(" mm3_min=", mm3_min);  //DEBUG
	direc>0 ? SERIAL_ECHOPGM(" " MSG_EN_AVANCE): SERIAL_ECHOPGM(" " MSG_EN_RETROCESO);
//*/
   
		WRITE(E0_DIR_PIN, dir);
		WRITE(E1_DIR_PIN, dir);		

//Calculamos los parametros para  1 mm3
	float mm3_t_useg = 1.00 * 60000000UL/ mm3_min;					// Tiempo en usegundos de la operacion para 1 mm3
	float upulse_C = 1.00 * planner.axis_steps_per_mm[E_AXIS]* mixing_factor[0];	//   micropasos/mm3 motor cartucho(estamos en modo volumetrico)
	float upulse_B = 1.00 * planner.axis_steps_per_mm[E_AXIS] * mixing_factor[1];	//   micropasos/mm3 motor boquilla 
	
	
	
/*	Calculamos los tramos buscando el menor multiplo para intentar conseguir la maxima simultaneidad de funcionamiento
	de ambos extrusores. 
	La variable <maxima_continuidad_mm3> es el limite de flujo admitido para un tramo y por tanto sera
	la maxima espera del otro extrusor.
	Calculamos el numero de pasos minimo <tramo_pasos> para el bucle comun
*/
	//tramo numero minimo de pasos por bucle secundario a i++.
	tramo_pasos = MIN(upulse_C,upulse_B) * MIN(pasta_mm3, maxima_continuidad_mm3);  
	 
	 //obtengo los limites de los tramos de los bucles minimos internos siendo el menor =1
	if (upulse_C >= upulse_B) { 
		paso_C= upulse_C/upulse_B;
		} else {
			paso_B= upulse_B/upulse_C;
			} 

	float stepDelay_B = mm3_t_useg * 0.5 / upulse_B;  //constante para cualquier cantidad de mm3
	float stepDelay_C = mm3_t_useg * 0.5 / upulse_C;  //constante para cualquier cantidad de mm3  

// los pasos totales a dar en cada extrusor:
	upulse_B *= pasta_mm3;
	upulse_C *= pasta_mm3;	
	
	if(motor!=0) WRITE(E1_ENABLE_PIN, LOW);   //enable motor del tornillo del cartucho
	if(motor!=1) WRITE(E0_ENABLE_PIN, LOW);   //enable motor del tornillo de la boquilla

  /*
	SERIAL_ECHOPAIR(" upulse_B=", upulse_B);
	SERIAL_ECHOPAIR(" upulse_C=", upulse_C);
	SERIAL_ECHOPAIR(" stepDelay_B=", stepDelay_B);	
	SERIAL_ECHOLNPAIR(" stepDelay_C=", stepDelay_C);	
		SERIAL_EOL();
  //*/

//	Hago ahora los ciclos de marcha combinada de los motores siempre !Importante! empezando por el CARTUCHO
//	
do 
{
	for (unsigned long i = 0; i < tramo_pasos; i++) {		
//		if 	FC_EXTR_TOP_ACTIVADO  stop();
		//primero el ciclo correspondiente al Cartucho
		for (unsigned long j = 0; j < paso_C; j++) {
			WRITE(E0_STEP_PIN, HIGH);
			delayMicroseconds(stepDelay_C);
			WRITE(E0_STEP_PIN, LOW);
			delayMicroseconds(stepDelay_C);
		}
		//segundo el ciclo correspondiente a la boquilla
		for ( unsigned long k = 0; k < paso_B; k++) {
			 WRITE(E1_STEP_PIN, HIGH);
			 delayMicroseconds(stepDelay_B);
			 WRITE(E1_STEP_PIN, LOW);
			 delayMicroseconds(stepDelay_B);
		}
	}
	// veo si he terminado 
	  upulse_C = upulse_C - paso_C * tramo_pasos;
	  upulse_B = upulse_B - paso_B * tramo_pasos;
	  resto = MIN(upulse_C,upulse_B);
 /*	  
	SERIAL_ECHOPAIR(" upulse_C=", upulse_C);	
	SERIAL_ECHOPAIR(" upulse_B=", upulse_B);
	SERIAL_ECHOLNPAIR(" tramo_pasos= ", tramo_pasos);
	SERIAL_ECHOLNPAIR(" resto= ", resto);
//*/	  
	  if (resto < tramo_pasos)  tramo_pasos = resto;
	  
}while (resto > 1);		
		
	if(motor!=1) WRITE(E0_ENABLE_PIN, HIGH);   //disable motor 
	if(motor!=0) WRITE(E1_ENABLE_PIN, HIGH);   //disable motor 
	
//	SERIAL_ECHOLNPAIR_F("  tiempo(s)= ", (float)(millis()-t_millis)/1000);

}  //FIN de extruir_tornillo
#endif
...
SERIAL_ECHO("\n");
SERIAL_ECHO("M163 >> ");
SERIAL_ECHOPAIR("E0=",mixing_factor[0]);
SERIAL_ECHOLNPAIR("   E1=",mixing_factor[1]);
}
...
inline void invalid_extruder_error(const uint8_t e) {
  ...
  SERIAL_ECHOLNPGM(MSG_INVALID_EXTRUDER);
	...
	inline void pe_activate_magnet(const uint8_t extruder_num) {
	  pe_set_magnet(extruder_num, PE_MAGNET_ON_STATE);
	}
	inline void pe_deactivate_magnet(const uint8_t extruder_num) {
	  pe_set_magnet(extruder_num, !PE_MAGNET_ON_STATE);
	}
...
#if ENABLED(PRESPRINT)
        case 10: gcode_G10(); break;                              // G10: Marcha Compresor
        case 11: gcode_G11(); break;                              // G11: Para Compresor
#endif
...
#if  ENABLED(PASTAPRINT_LIMPIEZA)
// GCODE NUEVO
        case 13: gcode_G13(); break;                              // G13: Limpieza de conductos y extrusor
#endif
        ...
        case 90: gcode_G90(); break;                      // G90: Absolute coordinates
        case 91: gcode_G91(); break;                       // G91: Relative coordinates
...
#if ENABLED(UVPRINT)  
        case 10: init_UVPRINT(); break;  //gcode UVPRINT  	 //	inicialización UVPRINT
#endif
#if ENABLED(PASTAPRINT)  
		case 8:	 gcode_M8(); break ;	 //Sonidos   S=secuencia  R=repeticion I=intervalo
		case 9:	 gcode_M9(); break ;	 //Coloca el carro en el centro X-Y 
        case 10: gcode_M10(); break;     //inicialización versiones PASTAPRINT
#endif
        case 11: gcode_M11(); break;     //gcode MULTIMAT_PRINTER// M11: Bajar la cama

#if ENABLED(PASTAPRINT_LIMPIEZA)       
		case 12: gcode_M12(); break;     // M12: Manejo de Servos
#endif

#if ENABLED(PASTAPRINT)    
		case 13: gcode_M13(); break;     // M13: Sangrado extrusor mediante gcode
		case 16: gcode_M16(); break;     // M16 "D" "F"  Set diametro boquilla y Factor de extrusion.

#endif															 //				Despues actualiza EEPROM  M500
...
#if ENABLED(PASTAPRINT)
//                           Gcodes para calibracion
		case 50: gcode_M50(); break;        // M50  'R' 'D' 'V' 'I' : Calibracion de rendimiento del extrusor
		case 51: gcode_M51(); break;        // M51  //     M15 V C N H X Y A B I J Calibracion de lineas 
#endif
...
#if ENABLED(UVPRINT)
		// estos son nuevos solo para UVPRINT
        case 703: encender_UV(); break;           // M703 enciende leds UV
		case 704: apagar_UV(); break;             // M704 apaga leds UV
#endif
...
//#if ENABLED(PASTAPRINT)
//   if(debug_SI) SERIAL_ECHO(current_command); //lista el gcode en el Terminal
//#endif
...
void prepare_move_to_destination() {
  ...
  if (
#if UBL_SEGMENTED				// es false, no es una DELTA
    ubl.prepare_segmented_line_to(destination, MMS_SCALED(feedrate_mm_s))
#elif IS_KINEMATIC
    prepare_kinematic_move_to(destination)
#else
    prepare_move_to_destination_cartesian()   //esto queda dentro del if(
#endif
  ) return;
...
//  SERIAL_ECHOLNPGM("INICIA SETUP. Inicializo pines");
//  SET_OUTPUT(SERVOS_PIN);
//  SET_OUTPUT(COMPRESOR_PIN);
//  SET_OUTPUT(BOMBA_AGUA_PIN);
  	  
SET_INPUT_PULLUP(BAJAR_CAMA_PIN);

#if ENABLED(UVPRINT)
	  SET_OUTPUT(RELES_UV_PIN);
	  DESACTIVAR_UV;
#endif

#if ENABLED(PASTAPRINT)
	  SET_INPUT_PULLUP(SENTIDO_AVANCE_MANUAL_PIN);
	  SET_INPUT_PULLUP(SANGRADO_MANUAL_PIN);
	  
	#if ENABLED(EMBOLO_TORNILLO)	
		SET_INPUT_PULLUP(FC_MORDAZA_ACCESIBLE_PIN);	
		SET_INPUT_PULLUP(LIBERAR_MORDAZA_PIN);			  
	#endif 
		  
	#if ENABLED(OBTU_SOLE)
	  	SET_OUTPUT(PWM_OBTURADOR_PIN);		
	  	digitalWrite(PWM_OBTURADOR_PIN,LOW);
		SET_INPUT_PULLUP(FC_OPEN_OBT_PIN); //normalmente HIGH (obturador abierto)
	#endif
	
	#if ENABLED(OBTU_TORN)
		SET_INPUT_PULLUP(HALF_PIN);
		SET_INPUT_PULLUP(OPEN_PIN);	
		SET_OUTPUT(IN1_PIN);
		SET_OUTPUT(IN2_PIN);		  
		digitalWrite(IN1_PIN, LOW);
		digitalWrite(IN2_PIN, LOW);
    ...
    #if ENABLED(EXTRUSOR_MIX)
		extrusor_mix_pastaprint();
	#endif


	#if  ENABLED(PRESPRINT)
		SET_OUTPUT(START_PIN);
		SET_INPUT(PRESION_PIN);	
		digitalWrite(START_PIN, LOW);
	#endif

#endif		//fin PASTAPRINT
...
#if ENABLED(FILAMENT_RUNOUT_SENSOR)		//$$ Puede usarse para que pida carga de material.
  runout.setup();
#endif
  ...
  //    disableStepperDrivers();   //PROVISIONAL
...
#if HAS_STEPPER_RESET
  MYSERIAL0.println("pasa por enableStepperDrivers()");
...
#if ENABLED(UVPRINT)
  reset_UVPRINT();
//  SERIAL_ECHOPGM("HAY PASTA: ");
//  (HAY_PASTA) ? SERIAL_ECHOLNPGM("Si"):SERIAL_ECHOLNPGM("No");
#endif
}
...
#if ENABLED(PASTAPRINT)
// Estas operaciones solo se pueden hacer si no estoy imprimiendo.
	if (!card.sdprinting) {
		if BAJAR_CAMA_ACTIVADO gcode_M11();

		if SANGRADO_MANUAL_ACTIVADO gcode_M13();		

		#if ENABLED(EMBOLO_TORNILLO) 
			if	LIBERAR_MORDAZA_ACTIVADO{  //si esta pulsado es LOW. Resistencia PullUP
					posicion_actuar_mordaza(150);  
					delay(200);
				}
			if (!ESTA_OPEN) abre_obturador();
		#endif
	}
if (obturador_time_fin_ms > millis()) abre_obturador();
#endif  //PASTAPRINT
  ...
  if (commands_in_queue) {
	...
	#if ENABLED(POWER_LOSS_RECOVERY)
		  if (card.cardOK && card.M) save_job_recovery_info();
	#endif
    }
