...
#define EEPROM_VERSION "V56" 		//Version 56 para UVPRINT dos datos al final (float y int8_t)
...
float diam_boquilla;
uint8_t factor_rendimiento_extrusion;
...
void MarlinSettings::postprocess() {
  ...
  bool MarlinSettings::save() {
	...
	#if  ENABLED(PASTAPRINT)	
		  EEPROM_WRITE(planner.diam_boquilla);	
		  EEPROM_WRITE(planner.factor_rendimiento_extrusion );
  ...
  bool MarlinSettings::_load() {
    ...
    if (strncmp(version, stored_ver, 3) != 0) {
	...
	#if  ENABLED(PASTAPRINT)	
		  EEPROM_READ(planner.diam_boquilla);
		  EEPROM_READ(planner.factor_rendimiento_extrusion );
	#endif	
	  ...
	  // nota de PASTAPRINT: como estamos usando modo VOLUMETRICO el valor de axis_steps_per_mm[3] que corresponde al extrusor esta en unidades de pasos/mm3
...
void MarlinSettings::reset() {
  ...
  #if  ENABLED(PASTAPRINT)
	planner.factor_rendimiento_extrusion  		= BQLLA_RENDIMIENTO_EXTRUSION ;
  #endif
  #if ENABLED(JUNCTION_DEVIATION)
  ...
  //
    ...
    for (uint8_t q = 0; q < COUNT(planner.filament_size); q++)
      planner.filament_size[q] = DIAMETRO_EMBOLO_MM;
  ...
  void MarlinSettings::report(const bool forReplay) {
	...
	if (parser.volumetric_enabled)   SERIAL_ECHOLNPGM("  MODO VOLUMETRICO (E mm3)");
 ...
 for (uint8_t i = 0; i < E_STEPPERS; i++) {
      ...
      SERIAL_ECHOPAIR(" E=", VOLUMETRIC_UNIT(planner.axis_steps_per_mm[E_AXIS + i]));
	  SERIAL_ECHOPGM(") ");
      }
*/
 	  SERIAL_ECHOPAIR(" (CTCHO T0 E=",  CTCHO_PASOS_MM3);
#if  ENABLED(EXTRUSOR_MIX)
 	  SERIAL_ECHOPAIR(") (BQLLA T1 E=",  BQLLA_PASOS_MM3_REALES);
 #endif
	  SERIAL_ECHOPGM(") ");	  	 
      SERIAL_EOL();
    #endif
    ...
    SERIAL_EOL();
      ...
      for (uint8_t i = 0; i < E_STEPPERS; i++) {
 	  SERIAL_ECHOPAIR(" (T", (int)i); 
      SERIAL_ECHOPAIR(" E=", VOLUMETRIC_UNIT(planner.max_feedrate_mm_s[E_AXIS + i]));
	  SERIAL_ECHOPGM(") ");
      }
      SERIAL_EOL();
    #endif
   ...
   SERIAL_EOL();
      ...
      for (uint8_t i = 0; i < E_STEPPERS; i++) {
 	  SERIAL_ECHOPAIR(" (T", (int)i); 
      SERIAL_ECHOPAIR(" E=", VOLUMETRIC_UNIT(planner.max_acceleration_mm_per_s2[E_AXIS + i]));
	  SERIAL_ECHOPGM(") ");
      }
      SERIAL_EOL();
    #endif
...
// Adicciones para  MULTIMAT_PRINTER. 	        
		  SERIAL_EOL();
		  CONFIG_ECHO_START;

		  SERIAL_ECHOLNPGM("\t\tAdiciones para MULTIMAT_PRINTER  ");
		  
	#if ENABLED(PRESPRINT) && ENABLED(OBTU_TORN)		  
		  SERIAL_ECHOLNPGM("\t\tVersion: AIRE COMPRIMIDO Y OBTURADOR POR TORNILLO Y MOTOR");
	#endif

	#if ENABLED(PRESPRINT) && ENABLED(OBTU_SOLE)		  
		  SERIAL_ECHOLNPGM("\t\tVersion: AIRE COMPRIMIDO Y OBTURADOR SOLENOIDE");
	#endif


// Dimensiones del espacio de impresion.
		  CONFIG_ECHO_START;
		  SERIAL_ECHO("\tDim.impresion>> ");
		  SERIAL_ECHOPAIR(" X=", X_BED_SIZE);
		  SERIAL_ECHOPAIR(" Y=", Y_BED_SIZE);
		  SERIAL_ECHOPAIR(" Z=", Z_MAX_POS);
		  SERIAL_EOL();

	#if ENABLED(EMBOLO_TORNILLO) 
		  CONFIG_ECHO_START;
		  SERIAL_ECHOPAIR("\tTransmisión: Zmotor(", Z_PINON_MOTOR);
		  SERIAL_ECHOPAIR(")/Ztornillo(", Z_RUEDA_TORNILLO);			
		  SERIAL_ECHOPAIR_F(")= ", (float)RELAC_MOTOR_TORNILLO);
		  SERIAL_ECHOLNPAIR("\tPaso tornillo= ", PASO_TORNILLO_MM);		  
	#endif		

		  
    #if  ENABLED(EXTRUSOR_MIX)
		extrusor_mix_pastaprint();
		SERIAL_ECHO("\tMixing Extrusores ");
		SERIAL_ECHO("M163 ");
		SERIAL_ECHOPAIR("(CTCHO)E0=",mixing_factor[0]);
		SERIAL_ECHOLNPAIR("  (BQLLA)E1=",mixing_factor[1]);
    #endif	
	
		  planner.actualiza_steps_mm3_E();  // refresco el dato	
		  
	#if ENABLED(EXTRUSOR_MIX)		  
		  SERIAL_ECHOPAIR_F("\tE1 >>" MSG_PASOS_MM3"(teor):", BQLLA_PASOS_MM3_TEOR);
		  SERIAL_ECHOPAIR("  " MSG_PORC_RENDMTO_EXTRUSION  ": M16 R<%Rendim =", planner.factor_rendimiento_extrusion );	  
		  SERIAL_ECHOPAIR_F(">  " MSG_PASOS_MM3 "(real):", planner.axis_steps_per_mm[E_AXIS+1]);
		  SERIAL_ECHOPAIR_F("   M16 D<Diam.boquilla =", DIAMETRO_EMBOLO_MM );
		  SERIAL_EOL();		  
	#endif	

	#if ENABLED(CTCHO_EMBOLO) 	  
		  CONFIG_ECHO_START;		  
		  SERIAL_ECHOPAIR_F("\tExtrusor: M16 D<" MSG_DIAM_EMBOLO ": ", planner.filament_size[0]);
		  SERIAL_ECHOPAIR_F(">  D<Diam." MSG_NOZZLE ": ", planner.diam_boquilla);	
		  SERIAL_ECHOPAIR(">  R<%Rend.: ", planner.factor_rendimiento_extrusion );	 		  
		  SERIAL_ECHO(">  G<" MSG_STORE_EEPROM ">");	
		  SERIAL_EOL();		  
	#endif	
	
		  CONFIG_ECHO_START;
		  SERIAL_ECHOLNPGM("\tLimpieza " MSG_NOZZLE ": G12 C<n_capas>");
			  
		  CONFIG_ECHO_START;
		  SERIAL_ECHOPAIR("\tSangrado: M13 S<", sangrado_mm3_min);
		  SERIAL_ECHOLNPGM("mm3/min> M<mm3> ");

		  CONFIG_ECHO_START;
		  SERIAL_ECHOLNPGM("\t" MSG_CALIBRACION ": " MSG_M50_CALIBRACION);
		  SERIAL_ECHOLNPGM("\t\t     "  MSG_M51_CALIBRACION1);
		  SERIAL_ECHOLNPGM("\t\t    " MSG_M51_CALIBRACION2);
		...
		abre_obturador();
			...
			SERIAL_ECHOPAIR("\t"MSG_MS_OBTURADOR "=", t_max_obturador_ms);
			SERIAL_ECHOPAIR("\t"MSG_MS_ENTRE_OPER_OBTURADOR "=", t_min_entre_operaciones_obturador_ms);
	#endif 
