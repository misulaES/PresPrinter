...
#include "point_t.h"
  ...
  void Nozzle::stroke(const point_t &start, const point_t &end, const uint8_t &strokes) {
	...
	// compruebo que Z >= Z_MIN_LIMPIA_BOQUILLA para evitar choques con el dispositivo de limpieza con cama cerca de Z=0
	//  y ademas para ajustar la altura adecuada de la superficie abrasiva de limpieza .
	  if (current_position[Z_AXIS] < Z_MIN_LIMPIA_BOQUILLA) do_blocking_move_to(start.x, start.y, Z_MIN_LIMPIA_BOQUILLA, 80); 
#else
	    do_blocking_move_to(start.x, start.y, start.z, 80);  //Modificacion. Pongo velocidad, si no  no funciona bien salta pasos
#endif

    // Start the stroke pattern
//    for (uint8_t i = 0; i < (strokes >> 1); i++) {
	 for (uint8_t i = 0; i < strokes ; i++) {
      do_blocking_move_to_xy(end.x, end.y, 30 );
      do_blocking_move_to_xy(start.x, start.y,30);
    }
