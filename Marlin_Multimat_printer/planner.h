...
class Planner {
  public:
    ...
    static float e_factor[EXTRUDERS];               // The flow percentage and volumetric multiplier combine
													//to scale E movement
 
 #if DISABLED(NO_VOLUMETRICS) 		//Es que estamos en modo VOLUMETRICS
 	...
 	static float diam_boquilla;				//diametro de la boquilla actual
	static uint8_t factor_rendimiento_extrusion ;    // Porcentaje del factor de eficiencia del extrusor de boquilla
 #endif
...
// la variable e_factor reune el %flow (y el volumetric_multiplier en caso de modo E NO V0LUMETRICO)
    FORCE_INLINE static void refresh_e_factor(const uint8_t e) {
      e_factor[e] = (flow_percentage[e] * 0.01f
  /*
	  Para PASTAPRINT el e_factor solo incluye el % de flujo que se asigne, ya que el eje E se ha 
	  cortado en mm3 y los pasos motor ya estan referidos a pasos por mm3.
  */
        #if DISABLED(NO_VOLUMETRICS) && DISABLED(PASTAPRINT)
    ...
    static void calculate_volumetric_multipliers();
      ...
      FORCE_INLINE static void set_filament_size(const uint8_t e, const float &v) {
        ...
        for (uint8_t i = 0; i < COUNT(filament_size); i++)
          if (!filament_size[i]) filament_size[i] = DIAMETRO_EMBOLO_MM;
      }
	 ...
	 #if ENABLED(PASTAPRINT)  
	 /*
	 actualiza con los actuales valores de 	axis_steps_per_mm[E_AXIS]  !ojo! en pasos/mm3
	 */
	  static void actualiza_steps_mm3_E();
    #endif
...
extern millis_t obturador_time_fin_ms;

	/* ****************************************** cierra_obturador() ****************************
	-Rutina para obturador tipo A/C (abierto/cerrado) de Solenoide. Normalmente abierto.
		Esta rutina cierra el paso en la boquilla actuando sobre el electroiman con corriente regulada por L298N
		el pin PWM_OBTURADOR_PIN que lo activa al pasar a HIGH. 
		Para evitar que se queme se controla que no este cerrado más de OBT_TIME_MAX_ACTIVO_S segundos continuadamente.
	*/
FORCE_INLINE  void cierra_obturador(){
		digitalWrite(PWM_OBTURADOR_PIN,HIGH);// signal to L298N Enable pin
		obturador_time_fin_ms = millis()+ OBT_TIME_MAX_ACTIVO_S * 1000;
}
	/* ****************************************** abre_obturador() ****************************
	Quitamos la corriente del solenoide poneniendo a cero PWM_OBTURADOR_PIN de forma que se libera (se abre) el obturador mediante resorte. Ajustar el delay segun pruebas
	*/
FORCE_INLINE void abre_obturador(){
		digitalWrite(PWM_OBTURADOR_PIN,LOW); // Disable electroiman
	}
#endif  //OBTU_SOLE
